...
int ff_AMediaCodecProfile_getProfileFromAVCodecContext(AVCodecContext *avctx)
{
    ...
    av_log(NULL,AV_LOG_DEBUG,"hongjian:avctx->profile=%d",avctx->profile);
    ...
    av_log(NULL,AV_LOG_DEBUG,"hongjian:field_id=%d",field_id);
        if (field_id) {
...
char *ff_AMediaCodecList_getCodecNameByType(const char *mime, int profile, int encoder, void *log_ctx)
{
    ...
    av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 1");
    if ((ret = ff_jni_init_jfields(env, &jfields, jni_amediacodeclist_mapping, 0, log_ctx)) < 0) {
        goto done;
    }
    av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 2");
    if ((ret = ff_jni_init_jfields(env, &mediaformat_jfields, jni_amediaformat_mapping, 0, log_ctx)) < 0) {
        goto done;
    }
    av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 3");
    codec_count = (*env)->CallStaticIntMethod(env, jfields.mediacodec_list_class, jfields.get_codec_count_id);
    if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
        goto done;
    }
    av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 4");
    for(i = 0; i < codec_count; i++) {
        ...
        av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 5 i=%d",i);
        types = (*env)->CallObjectMethod(env, info, jfields.get_supported_types_id);
        if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
            goto done;
        }
        av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 6 i=%d",i);
        is_encoder = (*env)->CallBooleanMethod(env, info, jfields.is_encoder_id);
        if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
            goto done;
        }
        av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 7 i=%d",i);
        if (is_encoder != encoder) {
            goto done_with_info;
        }
        av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 8 i=%d",i);
        ...
        for (j = 0; j < type_count; j++) {
            ...
            av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 9 j=%d",j);
            supported_type = ff_jni_jstring_to_utf_chars(env, type, log_ctx);
            if (!supported_type) {
                goto done;
            }
            av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 10 j=%d",j);
            if (!av_strcasecmp(supported_type, mime)) {
                codec_name = (*env)->CallObjectMethod(env, info, jfields.get_name_id);
                if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
                    goto done;
                }
                av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 11 j=%d",j);
                name = ff_jni_jstring_to_utf_chars(env, codec_name, log_ctx);
                if (!name) {
                    goto done;
                }
                av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 12 j=%d",j);
                if (strstr(name, "OMX.google")) {
                    av_freep(&name);
                    goto done_with_type;
                }
                av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 13 j=%d",j);
                capabilities = (*env)->CallObjectMethod(env, info, jfields.get_codec_capabilities_id, type);
                if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
                    goto done;
                }
                av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 14 j=%d",j);
                profile_levels = (*env)->GetObjectField(env, capabilities, jfields.profile_levels_id);
                if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
                    goto done;
                }
                av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 15 j=%d profile=%d",j,profile);
                profile_count = (*env)->GetArrayLength(env, profile_levels);
                if (!profile_count) {
                    found_codec = 1;
                }
                av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 15 j=%d profile_count=%d",j,profile_count);
                for (k = 0; k < profile_count; k++) {
                    ...
                    av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 16 k=%d",k);
                    profile_level = (*env)->GetObjectArrayElement(env, profile_levels, k);
                    if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
                        goto done;
                    }
                    av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 17 k=%d profile_level=%d",k,profile_level);
                    supported_profile = (*env)->GetIntField(env, profile_level, jfields.profile_id);
                    if (ff_jni_exception_check(env, 1, log_ctx) < 0) {
                        goto done;
                    }
                    av_log(NULL,AV_LOG_DEBUG,"hongjian: ff_AMediaCodecList_getCodecNameByType 18 k=%d supported_profile=%d",k,supported_profile);
