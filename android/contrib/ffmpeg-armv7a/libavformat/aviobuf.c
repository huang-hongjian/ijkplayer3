                  ...
                  int64_t (*seek)(void *opaque, int64_t offset, int whence))
{
    ...
    s->pos             = 0;
    s->eof_reached     = 0;
    av_log(NULL,AV_LOG_DEBUG,"ffio_init_context eof_reached=0");
    s->error           = 0;
...
int64_t avio_seek(AVIOContext *s, int64_t offset, int whence)
{
    ...
    if ((!s->direct || !s->seek) &&
        ...
        s->pos = pos;
        s->eof_reached = 0;
        av_log(NULL,AV_LOG_DEBUG,"avio_seek eof_reached=0");
        fill_buffer(s);
    ...
    s->eof_reached = 0;
    av_log(NULL,AV_LOG_DEBUG,"avio_seek2 eof_reached=0");
    return offset;
}
...
static void fill_buffer(AVIOContext *s)
{
    ...
    {    
        s->eof_reached = 1;
       // av_log(NULL,AV_LOG_DEBUG,"fill_buffer1-----> eof_reached=1");
    }
    ...
    if (s->read_packet)
    {
       ...
       // av_log(NULL,AV_LOG_DEBUG,"fill_buffer-------> read_packet=%p len=%d",s->read_packet,len);
    ...
    if (len <= 0) {
        ...
        s->eof_reached = 1;
       // av_log(NULL,AV_LOG_DEBUG,"fill_buffer2----> eof_reached=1");
        if (len < 0)
            s->error = len;
    } else {
        s->pos += len;
...
int avio_read(AVIOContext *s, unsigned char *buf, int size)
{
    ...
    size1 = size;
    while (size > 0) {
        //av_log(NULL,AV_LOG_DEBUG,"avio_read  while---->");
        ...
        if (len == 0 || s->write_flag) {
            if((s->direct || size > s->buffer_size) && !s->update_checksum) {
                ...
                if(s->read_packet)
                {
                      ...
                      //av_log(NULL,AV_LOG_DEBUG,"avio_read-------> read_packet=%p len=%d",s->read_packet,len);
                }
                ...
                if (len <= 0) {
                    ...
                    be done without rereading data */
                    s->eof_reached = 1;
                    //av_log(NULL,AV_LOG_DEBUG,"avio_read eof_reached=1");
                    if(len<0)
                        s->error= len;
                    break;
                } else {
                ...
                //av_log(NULL,AV_LOG_DEBUG,"avio_read------->fill_buffer");
                fill_buffer(s);
                len = s->buf_end - s->buf_ptr;
                if(s->error == AVERROR_NET_DOWN)
                {
                    s->eof_reached = 0;
                    s->error = 0;
                }
               // av_log(NULL,AV_LOG_DEBUG,"avio_read------->fill_buffer  len=%d error = %d",len,s->error);
                if (len == 0)
                    break;
            }
        } else {
            //av_log(NULL,AV_LOG_DEBUG,"avio_read------->memcpy");
    ...
    if (size1 == size) {
       // av_log(NULL,AV_LOG_DEBUG,"avio_read-------> error=%d",s->error);
        if (s->error)      return s->error;
        if (avio_feof(s))  return AVERROR_EOF;
    }
   // av_log(NULL,AV_LOG_DEBUG,"avio_read-------> size1=%d size=%d",size1,size);
    return size1 - size;
}
...
static int io_read_packet(void *opaque, uint8_t *buf, int buf_size)
{
   ...
   // av_log(NULL,AV_LOG_DEBUG,"io_read_packet ------> %p",(void*)io_read_packet);
...
int ffio_rewind_with_probe_data(AVIOContext *s, unsigned char **bufp, int buf_size)
{
    ...
    s->eof_reached = 0;
    av_log(NULL,AV_LOG_DEBUG,"ffio_rewind_with_probe_data eof_reached=0");
