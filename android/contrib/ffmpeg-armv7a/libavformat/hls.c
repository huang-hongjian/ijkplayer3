...
struct playlist {
    ...
    int64_t time_shift_seek_timestamp;
    int seek_flags;
                    ...
                    AVDictionary *opts, AVDictionary *opts2, int *is_http)
{
    ...
    av_log(NULL,AV_LOG_DEBUG,"open_url-----> url=%s",url);
    if (ret >= 0) {
...
static int64_t parse_utc_time(char* utc)
{
    //#EXT-X-PROGRAM-DATE-TIME:2019-01-25T02:39:10Z
    char *ptr_temp = utc;
    struct tm* tmp_time ;
    time_t t;
    int64_t ret = 0;
    int64_t time_tmp = 0;
    // 2018-02-06T00:54:13Z
    while (av_isspace(*ptr_temp) || *ptr_temp == ':')
        ptr_temp++;
    //av_log(NULL,AV_LOG_DEBUG,"parse_utc_time %s",ptr_temp);
    strptime(ptr_temp,"%Y-%m-%dT%H:%M:%SZ",&tmp_time);
    t = mktime(&tmp_time);//时区问题
    t = t ;
    
    time_tmp = av_gettime()/1000000LL;
    //av_log(NULL,AV_LOG_DEBUG,"parse_utc_time time_tmp=%lld t=%ld",time_tmp,t);
    if(t > time_tmp + 24*60*60LL || t < time_tmp - 24*60*60LL)//one day
        return AV_NOPTS_VALUE;
    ret = t;
    ret = ret*1000000LL;
    //av_log(NULL,AV_LOG_DEBUG,"parse_utc_time %lld",ret);
    //printf( "%4d-%2d-%2dT%2d:%2d:%2dZ",
    //    tmp_time.year, tmp_time.month,
    //    tmp_time.day, tmp_time.hour,
    //    tmp_time.min, tmp_time.sec);
    //*current_segment_sec = utc_clock->hour*3600 + utc_clock->min*60 + utc_clock->sec;   
    return ret;
}
                          ...
                          struct playlist *pls, AVIOContext *in)
{
    ...
    int64_t duration = 0, previous_duration1 = 0, previous_duration = 0, total_duration = 0,utc_time = 0;
    ...
    if (!in) {
        ...
        av_log(NULL,AV_LOG_DEBUG,"%s %d io_open ret=%d",__FUNCTION__,__LINE__,ret);
        av_dict_free(&opts);
        if (ret < 0)
            return ret;
#else
    ...
    if (pls) {
        av_log(NULL,AV_LOG_DEBUG,"hongjian:parse_playlist index=%d",pls->index);
        free_segment_list(pls);
        pls->finished = 0;
    ...
    while (!avio_feof(in)) {
        read_chomp_line(in, line, sizeof(line));
        av_log(NULL,AV_LOG_DEBUG,"hongjian:%s",line);
...
static int update_init_section(struct playlist *pls, struct segment *seg)
{
    ...
    int64_t urlsize;
    int ret;
    av_log(NULL,AV_LOG_DEBUG,"%s %d ------>init_section=%p cur_init_section=%p",__FUNCTION__,__LINE__,
        seg->init_section,pls->cur_init_section);
...
static int buld_time_shift_url(char* url , char* out_url,int64_t offset_time)
{
    if(!url || !out_url)
        return -1;
    ...
    int64_t seek_time = (current_time - offset_time)/1000000-60LL;
    long seek_time_tmp = (long)seek_time;
    av_log(NULL,AV_LOG_DEBUG,"buld_time_shift_url current_time=%lld seek_time=%lld offset_time=%lld",current_time,seek_time,offset_time);
    time_t t;
    struct tm *lt;
    t=seek_time ;
    lt=localtime(&t);
    char s[100] = {0};
    sprintf(s,"playseek=%04d%02d%02d%02d%02d%02d-",
        lt->tm_year+1900,lt->tm_mon+1,lt->tm_mday,
        lt->tm_hour,lt->tm_min,lt->tm_sec);
    //strftime(s, sizeof(s), "playseek=%Y%m%d%H%M%S-", p);
    av_log(NULL,AV_LOG_DEBUG,"buld_time_shift_url %s t=%ld seek_time_tmp=%ld",s,t,seek_time_tmp);
    char * pos = strstr(url,"playseek=");
    int end_len = pos?strlen(pos):0;
    //playseek=20190125154800-
    int tmp_len = strlen("playseek=20190125154800-");
    if(pos && end_len>0 &&  end_len >= tmp_len && pos[tmp_len-1] == '-')
    {
        int start_len = (int)pos - (int)url;
        int end_len = strlen(pos[tmp_len]);
        memcpy(out_url,url,start_len);
        memcpy(out_url+start_len,s,tmp_len);
        if(end_len)
            memcpy(out_url+start_len+tmp_len,pos[tmp_len-1],end_len);
        av_log(NULL,AV_LOG_DEBUG,"buld_time_shift_url 0 out_url=%s",out_url);
    ...
    else
    {
        if(url[url_len-1] == '&')
        {
            sprintf(out_url, "%s%s&", url, s);
            av_log(NULL,AV_LOG_DEBUG,"buld_time_shift_url 1 out_url=%s",out_url);
        }
        else
        {
            sprintf(out_url, "%s&%s", url, s);
            av_log(NULL,AV_LOG_DEBUG,"buld_time_shift_url &out_url=%s",out_url);
        }
...
static int read_data(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    int reload_count = 0;
    int reload_m3u8 = 0;
    int open_input_ts_err = 0;
    int64_t last_load_time = 0;
restart:
    //av_log(NULL,AV_LOG_DEBUG,"read_data-------> read_data=%p reload_count=%d max_reload=%d needed=%d index=%d",(void*)read_data,reload_count,c->max_reload,v->needed,v->index);
    if (!v->needed)
        return AVERROR_EOF;
    ...
    if (!v->input) {
        ...
        if (!v->needed) {
            av_log(v->parent, AV_LOG_DEBUG, "No longer receiving playlist---------> %d\n",
                v->index);
            return AVERROR_EOF;
        }
...
reload:
    //av_log(NULL,AV_LOG_DEBUG,"%s %d ---->reload_count=%d",__FUNCTION__,__LINE__,reload_count);
        reload_count++;
        if(reload_count > c->max_reload && open_input_ts_err == 1)
            return AVERROR_NET_DOWN;
...
reparse_playlist:
        if (!v->finished &&
            ...
            char url[2048]={0};
            if(c->is_time_shift && v->time_shift_seek_timestamp > 0)
            {
               if(buld_time_shift_url(v->url , url,v->time_shift_seek_timestamp) < 0)
                   strcpy(url,v->url);
            }
            else
                strcpy(url,v->url);
            if ((ret = parse_playlist(c, url, v, NULL)) < 0) {
                av_log(v->parent, AV_LOG_DEBUG, "Failed to reload playlist-------> %d  ret=%d\n",
                       v->index,ret);
                if(ret == -5 || ret == -111 || ret == -101)
                {
                    reload_m3u8 = 1;
                    v->last_load_time = av_gettime_relative();
                    goto reparse_playlist;
                }
                return ret;
            }
        ...
        else if(reload_m3u8 == 1 && !v->finished)
        {
             usleep(25000);
        ...
        //av_log(NULL,AV_LOG_DEBUG,"%s %d ---->cur_seq_no=%d",__FUNCTION__,__LINE__,v->cur_seq_no);
        ...
        if (ret)
        {
            av_log(NULL,AV_LOG_DEBUG,"%s %d ret=%d",__FUNCTION__,__LINE__,ret);
            return ret;
        }
        

        if(open_input_ts_err == 1)
        {
            usleep(25*1000);
            if (ff_check_interrupt(c->interrupt_callback))
                return AVERROR_EXIT;
            usleep(25*1000);
            if((av_gettime_relative() - last_load_time) >= reload_interval)
        ...
        av_log(NULL,AV_LOG_DEBUG,"%s %d----------> ret=%d cur_seq_no=%d reload_interval=%lld",__FUNCTION__,__LINE__,ret,v->cur_seq_no,reload_interval);
        if (ret < 0) {
            if (ff_check_interrupt(c->interrupt_callback))
                return AVERROR_EXIT;
            ...
            {
                open_input_ts_err =1;
                last_load_time = av_gettime_relative();
            }
            goto reload;
        }
        just_opened = 1;
    ...
    if (v->init_sec_buf_read_offset < v->init_sec_data_len) {
        ...
        av_log(NULL,AV_LOG_DEBUG,"%s %d ------------->copy_size=%d",__FUNCTION__,__LINE__,copy_size);
        return copy_size;
    }
    av_log(NULL,AV_LOG_DEBUG,"%s %d ---->",__FUNCTION__,__LINE__);
    ...
    av_log(NULL,AV_LOG_DEBUG,"%s %d restart----> ret=%d cur_seq_no=%d",__FUNCTION__,__LINE__,ret,v->cur_seq_no);
    goto restart;
}
                                      ...
                                      int64_t timestamp, int *seq_no)
{
    ...
    int64_t pos = c->first_timestamp == AV_NOPTS_VALUE ?
                  0 : c->first_timestamp;
    av_log(NULL,AV_LOG_DEBUG,"find_timestamp_in_playlist pls->finished=%d",pls->finished);
    if(pls->finished == 0)
    {
        if (timestamp <= 0) {
        ...
        int64_t current_utc_time = av_gettime();
        current_utc_time = current_utc_time - 8*60*60*1000000LL;

        for (i = 0; i < pls->n_segments; i++) {
            //av_log(NULL,AV_LOG_DEBUG,"find_timestamp_in_playlist current_utc_time=%lld utc_time=%lld av_gettime=%lld",current_utc_time,pls->segments[i]->utc_time,av_gettime());
            if(pls->segments[i]->utc_time == AV_NOPTS_VALUE)
                continue;
            int64_t diff1 = current_utc_time - timestamp;
            int64_t diff2 = diff1 - pls->segments[i]->utc_time;
            int64_t diff3 = 0LL;
            if(i != pls->n_segments -1)
                diff3 = pls->segments[i+1]->utc_time - diff1;
            else
                pls->segments[i]->utc_time+pls->segments[i]->duration+500000LL -diff1;
            av_log(NULL,AV_LOG_DEBUG,"find_timestamp_in_playlist diff2=%lld diff3=%lld",diff2,diff3);;
            //int flag = (current_utc_time - timestamp >= pls->segments[i]->utc_time) && (current_utc_time - timestamp <= pls->segments[i]->utc_time + pls->segments[i]->duration );
            //int64_t diff = (pls->segments[i]->utc_time + pls->segments[i]->duration + timestamp) - current_utc_time ;
            //av_log(NULL,AV_LOG_DEBUG,"find_timestamp_in_playlist utc_time=%lld duration=%lld timestamp=%lld diff=%lld",
            //    pls->segments[i]->utc_time,pls->segments[i]->duration,timestamp,diff);
            if (diff2 >= 0 && diff3 > 0) {
                *seq_no = pls->start_seq_no + i;
                av_log(NULL,AV_LOG_DEBUG,"find_timestamp_in_playlist seq=%d",pls->start_seq_no + i);
                return 1;
            }
        }        
    ...
    for (i = 0; i < c->n_variants; i++) {
        ...
        for (j = 0; j < v->n_playlists; j++) {
            if (v->playlists[j] != pls)
                continue;
                av_log(NULL,AV_LOG_DEBUG,"hongjian: %s %d bandwidth=%d stream=%p [i=%d][j=%d]",__FUNCTION__,__LINE__,v->bandwidth,stream,i,j);
...
{
    //http://117.169.120.190:8080/wh7f454c46tw887076299_72956127/PLTV/88888888/224/3221225578/10000100000000060000000000000030_0.smil/index.m3u8?playseek=20190125100000&fmt=ts2hls&servicetype=1&icpid=SSPID&accounttype=1&limitflux=-1&limitdur=-1&GuardEncType=2&accountinfo=VFyr4N4TxEAHZaL3syi9wI9%2FnLj3ei3x2vZgR6NR9qCWim3rYamnvEfnywl3DxJv%2BfjXTKQUGwOMzoJ913c3rrXbTwAo7Z7dDSOVTzoM4ilfwAX6kOLv6bPzScdMa%2B3Lwk4d0Z9iYns3S9BoAoGprA%3D%3D%3A20190125093214%2Cjiuliantest1%2C120.198.34.172%2C20190125093214%2C10000100000000050000000000000028%2C6E5A5F53EA3DAC5CFE9EF9812CBBD5C5%2C-1%2C0%2C1%2C%2C%2C2%2C%2C%2C%2C2%2CEND&RTS=1548382809&from=1&hms_devid=83
    //http://117.169.120.190:8080/wh7f454c46tw887076299_72956127/PLTV/88888888/224/3221225578/10000100000000060000000000000030_0.smil/01.m3u8?fmt=ts2hls,83,01.m3u8,0,2744,0,0&playseek=20190125100000-
    void *u = (s->flags & AVFMT_FLAG_CUSTOM_IO) ? NULL : s->pb;
    HLSContext *c = s->priv_data;
    int ret = 0, i;
    int highest_cur_seq_no = 0;
    av_log(NULL,AV_LOG_DEBUG,"hongjian %s %d s->pb=%p",__FUNCTION__,__LINE__,s->pb);
    c->ctx                = s;
    ...
    av_log(NULL,AV_LOG_DEBUG,"hongjian: %s %d n_variants=%d",__FUNCTION__,__LINE__,c->n_variants);
    ...
    for (i = 0; i < c->n_playlists; i++) {
        struct playlist *pls = c->playlists[i];
        av_log(NULL,AV_LOG_DEBUG,"hongjian: %s %d playlists[%d].n_segments=%d",__FUNCTION__,__LINE__,i,pls->n_segments);
        ...
        av_log(NULL,AV_LOG_DEBUG,"hongjian: %s %d playlists[%d].highest_cur_seq_no=%d  cur_seq_no=%d",__FUNCTION__,__LINE__,i,highest_cur_seq_no,pls->cur_seq_no);
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (pls->n_segments == 0)
            continue;
		pls->ctx->flags |= AVFMT_FLAG_KEEP_SIDE_DATA;
        ...
        av_log(NULL,AV_LOG_DEBUG,"hongjian:avformat_open_input index=%d",pls->index);
...
static int recheck_discard_flags(AVFormatContext *s, int first)
{
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (pls->cur_needed && !pls->needed) {
            ...
            av_log(NULL,AV_LOG_DEBUG,"recheck_discard_flags eof_reached=0");
...
static int hls_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        if (pls->needed && !pls->pkt.data) {
            while (1) {
                ...
                //av_log(NULL,AV_LOG_DEBUG,"hongjian: %s %d ret=%d i=%d size=%d",__FUNCTION__,__LINE__,ret,i,pls->pkt.size);
                if (ret < 0) {
                    if(s->pb && ret == AVERROR(EIO))
                    {
                        s->pb->error = ret;
    ...
    //av_log(NULL,AV_LOG_DEBUG,"hongjian: %s %d minplaylist=%d",__FUNCTION__,__LINE__,minplaylist);
    //minplaylist = 2;
                               ...
                               int64_t timestamp, int flags)
{
    ...
    int64_t first_timestamp, seek_timestamp, duration,seek_timestamp_offset;
    av_log(NULL,AV_LOG_DEBUG,"hls_read_seek stream_index=%d timestamp=%lld first_timestamp=%lld",stream_index,timestamp,c->first_timestamp);
    if(s->is_hls_vod == 0)
    {
         int64_t seek_timestamp_tmp = av_rescale_q(timestamp,s->streams[stream_index]->time_base,AV_TIME_BASE_Q);
         if(seek_timestamp_tmp == AV_NOPTS_VALUE)
            return AVERROR(EIO);
         seek_timestamp_offset = seek_timestamp_tmp - s->start_time;
         av_log(NULL,AV_LOG_DEBUG,"hls_read_seek stream_index=%d seek_timestamp_offset=%lld start_time=%lld seek_timestamp_tmp=%lld",stream_index,seek_timestamp_offset, s->start_time,seek_timestamp_tmp);
    ...
    {
        first_timestamp = 0;
        seek_timestamp = seek_timestamp_offset ;
        duration = s->duration ;
        av_log(NULL,AV_LOG_DEBUG,"hls_read_seek seek_timestamp=%lld duration=%lld first_timestamp=%lld",seek_timestamp,duration,first_timestamp);
        if (0 < duration && seek_timestamp <0 && duration < seek_timestamp - first_timestamp)
            return AVERROR(EIO);
    }
    ...
    if(s->is_hls_vod == 0 && seek_pls)
    {
        char url[2048]={0};
        if(c->is_time_shift && seek_timestamp > 0)
        {
           if(buld_time_shift_url(seek_pls->url , url,seek_timestamp) < 0)
               strcpy(url,seek_pls->url);
           seek_pls->time_shift_seek_timestamp = seek_timestamp;
        }
        else
            strcpy(url,seek_pls->url);
        if(parse_playlist(c, url, seek_pls, NULL) < 0)
            return AVERROR(EIO);
    }
    av_log(NULL,AV_LOG_DEBUG,"hls_read_seek seek_pls=%p",seek_pls);
    /* check if the timestamp is valid for the playlist with the
     * specified stream index */
    if (!seek_pls || !find_timestamp_in_playlist(c, seek_pls, seek_timestamp, &seq_no))
        return AVERROR(EIO);
    av_log(NULL,AV_LOG_DEBUG,"hls_read_seek seq_no=%d",seq_no);
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        reset_packet(&pls->pkt);
        pls->pb.eof_reached = 0;
        av_log(NULL,AV_LOG_DEBUG,"hls_read_seek eof_reached=0");
