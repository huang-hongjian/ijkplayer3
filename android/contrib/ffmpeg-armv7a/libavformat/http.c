...
static int http_open_cnx_internal(URLContext *h, AVDictionary **options)
{
    ...
    av_log(NULL,AV_LOG_DEBUG,"http_open_cnx_internal h=%x------>buf=%s rw_timeout=%d",h,buf,h->rw_timeout);

    if(!strstr(buf,"&") && !strstr(buf,"?")  &&  h->rw_timeout !=0)
    {
        char time[50]={0};
        sprintf(time,"?timeout=%d",h->rw_timeout);
        strcat(buf,time);
    }
    else if(strstr(buf,"?") && h->rw_timeout != 0)
    {
        char time[50]={0};
        sprintf(time,"&timeout=%d",h->rw_timeout);
        strcat(buf,time);
    ...
    av_log(NULL,AV_LOG_DEBUG,"http_open_cnx_internal ------>buf=%s rw_timeout=%d",buf,h->rw_timeout);
    if (!s->hd) {
...
redo:
    ...
    av_log(NULL,AV_LOG_DEBUG,"%s %d ---->http_code=%d location_changed=%d",__FUNCTION__,__LINE__,s->http_code,location_changed);
