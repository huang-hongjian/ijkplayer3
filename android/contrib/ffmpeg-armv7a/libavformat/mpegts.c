...
#define TS_FILE_DEBUG 		0

#if TS_FILE_DEBUG
#include <sys/system_properties.h>

#define TS_FILE_PATH 			"/sdcard/ts_debug.ts"
#define TS_FILE_DEFAULT_SIZE 	100*1024*1024
#define TS_DEBUG_PROPERTY 		"persist.um.player.debug"
#define TS_DEBUG_SIZE_PROPERTY	"persist.um.player.debug_size"

#define property_get(key, value) __system_property_get(key, value);
#endif
...
#if TS_FILE_DEBUG
static void open_ts_file(MpegTSContext *ts)
{
    char ts_debug[10];
	char ts_max_size[10];
	property_get(TS_DEBUG_PROPERTY, ts_debug);
	property_get(TS_DEBUG_SIZE_PROPERTY, ts_max_size);
	if(ts_debug != NULL && strcmp(ts_debug, "true") == 0) {
		if(access(TS_FILE_PATH, F_OK) == 0)
		remove(TS_FILE_PATH);
		ts->ts_file = fopen(TS_FILE_PATH, "wb+");
		ts->ts_file_size = 0;
		
		if(ts_max_size != NULL && strtol(ts_max_size, NULL, 10) > (TS_FILE_DEFAULT_SIZE/(1024*1024)))
			ts->ts_file_max_size = strtol(ts_max_size, NULL, 10)*1024*1024;
		else
			ts->ts_file_max_size = TS_FILE_DEFAULT_SIZE;
	} else {
		ts->ts_file = NULL;
	}
}

static void write_buffer_to_ts_file(MpegTSContext *ts, const uint8_t *packet)
{
    const uint8_t *packet_temp = packet;
    if(ts->ts_file != NULL) {
		fwrite(packet_temp, 1, 188, ts->ts_file);
		ts->ts_file_size += 188;
		if(ts->ts_file_size > ts->ts_file_max_size) {
			fclose(ts->ts_file);
			ts->ts_file = NULL;
		}
	}
...
static void close_ts_file(MpegTSContext *ts)
{
    if(ts->ts_file != NULL) {
		fclose(ts->ts_file);
		ts->ts_file = NULL;
...
static void mpegts_close_filter(MpegTSContext *ts, MpegTSFilter *filter)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "mpegts_close_filter--> \n");
    av_free(filter);
    ts->pids[pid] = NULL;
}
                            ...
                            int64_t pos)
{
    ...
    if (!ts->pkt)
        return 0;
    //av_log(NULL, AV_LOG_DEBUG, "%s %d is_start=%d  pes->state=%d pes->data_index=%d",__FUNCTION__,__LINE__ ,is_start,pes->state,pes->data_index);
    if (is_start) {
        if (pes->state == MPEGTS_PAYLOAD && pes->data_index > 0) {
            ret = new_pes_packet(pes, ts->pkt);
            //av_log(NULL, AV_LOG_DEBUG, "%s %d ret=%d ts->stop_parse = 1; ",__FUNCTION__,__LINE__,ret );
            if (ret < 0)
                return ret;
            ts->stop_parse = 1;
        } else {
    ...
    while (buf_size > 0) {
        ...
        case MPEGTS_HEADER:
            ...
            pes->data_index += len;
            //av_log(NULL, AV_LOG_DEBUG, "%s %d pes->data_index=%d ",__FUNCTION__,__LINE__,pes->data_index );
            p += len;
            buf_size -= len;
            if (pes->data_index == PES_START_SIZE) {
                /* we got all the PES or section header. We can now
                 * decide */
                //av_log(NULL, AV_LOG_DEBUG, "%s %d %02x%02x%02x",__FUNCTION__,__LINE__,pes->header[0],pes->header[1],pes->header[2]);
                if (pes->header[0] == 0x00 && pes->header[1] == 0x00 &&
                    pes->header[2] == 0x01) {
                    //av_log(NULL, AV_LOG_DEBUG, "%s %d pid=%x pes_code=%#x\n",__FUNCTION__,__LINE__,pes->pid,code);
                    ...
                    //if(pes->st )
                    //av_log(NULL, AV_LOG_DEBUG, "%s %d code=%03x st->index=%d",__FUNCTION__,__LINE__,code,pes->st->index);
        ...
        case MPEGTS_PAYLOAD:
            if (pes->buffer) {
                if (pes->data_index > 0 &&
                    ...
                    if (!pes->buffer)
                        return AVERROR(ENOMEM);
                    //av_log(NULL, AV_LOG_DEBUG, "%s %d ts->stop_parse = 1; ",__FUNCTION__,__LINE__ );
                    ts->stop_parse = 1;
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for (;;) {
        ...
        if (stream_type < 0)
            break;
        pid = get16(&p, p_end);
        av_log(NULL, AV_LOG_DEBUG, "%s %d pid = %x ",__FUNCTION__,__LINE__,pid );
        if (pid < 0)
            goto out;
        pid &= 0x1fff;
...
static void pat_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for (;;) {
        ...
        av_log(ts->stream, AV_LOG_DEBUG, "pat_cb sid=0x%x pid=0x%x\n", sid, pmt_pid);
        ...
        if (sid == 0x0000) {
            ...
            if (fil)
                {
                av_log(NULL, AV_LOG_DEBUG, "pat_cb section_cb=0x%x fil->pid=0x%x\n", fil->u.section_filter.section_cb, fil->pid);
            ...
            if(ts->pids[pmt_pid])
                {
               mpegts_close_filter(ts, ts->pids[pmt_pid]);
                 MpegTSFilter* filter = mpegts_open_section_filter(ts, pmt_pid, pmt_cb, ts, 1);
                 av_log(NULL, AV_LOG_DEBUG, "mpegts_open_section_filter1--> filter=%p\n",filter);
            }
            if (!ts->pids[pmt_pid])
             {
                 MpegTSFilter* filter = mpegts_open_section_filter(ts, pmt_pid, pmt_cb, ts, 1);
                 av_log(NULL, AV_LOG_DEBUG, "mpegts_open_section_filter--> filter=%p\n",filter);
             }
...
static int handle_packet(MpegTSContext *ts, const uint8_t *packet)
{
    ...
    int64_t pos;
#if TS_FILE_DEBUG
    write_buffer_to_ts_file(ts, packet);
#endif
    //av_printf_byte("",packet, 16);
                       ...
                       const uint8_t **data)
{
    ...
    for (;;) {
        //av_log(NULL,AV_LOG_DEBUG,"read_packet ----->");
        len = ffio_read_indirect(pb, buf, TS_PACKET_SIZE, data);
        //av_log(NULL,AV_LOG_DEBUG,"read_packet ----->len=%d",len);
                               ...
                               const uint8_t *buf, int len)
{
    ...
    for (;;) {
        ...
        //if(len>8)
        //    av_printf_byte("", buf, 8);
        //av_log(NULL, AV_LOG_DEBUG, "%s %d len =%d ",__FUNCTION__,__LINE__,len );
    }
    return len1 - len;
}
