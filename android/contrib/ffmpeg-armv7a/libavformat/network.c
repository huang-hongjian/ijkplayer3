...
int ff_network_wait_fd_timeout(int fd, int write, int64_t timeout, AVIOInterruptCB *int_cb)
{
    ...
    while (1) {
        ...
        //av_log(NULL,AV_LOG_DEBUG,"ff_network_wait_fd_timeout -------->ret=%d",ret);
...
av_log(NULL, AV_LOG_ERROR, "ff_poll_interrupt-------1--------->ret=%d %d",ret,AVERROR(ETIMEDOUT));
    if (!ret)
        return AVERROR(ETIMEDOUT);
    if (ret < 0)
        return ff_neterrno();
                      ...
                      int will_try_next)
{
    ...
    while ((ret = connect(fd, addr, addrlen))) {
        av_log(h, AV_LOG_ERROR, "connect-------1--------->ret=%d", ret);
        ret = ff_neterrno();
        switch (ret) {
        case AVERROR(EINTR):
            if (ff_check_interrupt(&h->interrupt_callback))
                return AVERROR_EXIT;
            av_log(h, AV_LOG_ERROR, "connect-------1--------->");
            continue;
        case AVERROR(EINPROGRESS):
        case AVERROR(EAGAIN):
            av_log(h, AV_LOG_ERROR, "connect-------1--------->%d %d %d %d",AVERROR(EINPROGRESS),AVERROR(EAGAIN),ret,timeout);
        ...
        default:
            return ret;
            av_log(h, AV_LOG_ERROR, "connect--------2-------->");
        }
    }
    return ret;
}
