                                     ...
                                     const uint8_t *buf, int len)
{
    ...
    if (s->handler && s->handler->parse_packet) {
        rv = s->handler->parse_packet(s->ic, s->dynamic_protocol_context,
                                      ...
                                      flags);
        //av_log(s, AV_LOG_DEBUG, "%s %d rv=%d",__FUNCTION__,__LINE__,rv);
    } else if (st) {
                                ...
                                uint8_t **bufptr, int len)
{
    ...
    uint32_t timestamp;
    int rv = 0;
    //av_log(s, AV_LOG_DEBUG, "%s %d",__FUNCTION__,__LINE__);
    if (!buf) {
    ...
    if ((s->seq == 0 && !s->queue) || s->queue_size <= 1) {
        ...
        uint16_t seq = AV_RB16(buf + 2);
        av_log(NULL, AV_LOG_DEBUG, "rtp_parse_one_packet--> seq =%d\n",seq );
                        ...
                        uint8_t **bufptr, int len)
{
    ...
    if (s->srtp_enabled && bufptr && ff_srtp_decrypt(&s->srtp, *bufptr, &len) < 0)
        return -1;
    //av_log(s, AV_LOG_DEBUG, "rtp_parse_one_packet--> \n");
    rv = rtp_parse_one_packet(s, pkt, bufptr, len);
    //av_log(s, AV_LOG_DEBUG, "rtp_parse_one_packet<-- rv=%d \n",rv);
    s->prev_ret = rv;
