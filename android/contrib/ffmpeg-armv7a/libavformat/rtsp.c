...
#if CONFIG_RTSP_DEMUXER
    case RTSP_LOWER_TRANSPORT_TCP:
        //av_log(s, AV_LOG_DEBUG, "ff_rtsp_tcp_read_packet--> \n");
        len = ff_rtsp_tcp_read_packet(s, rtsp_st, rt->recvbuf, RECVBUF_SIZE);
        //if(len >16)av_printf_byte("ff_rtsp_tcp_read_packet",rt->recvbuf, 16);
        //av_log(s, AV_LOG_DEBUG, "ff_rtsp_tcp_read_packet<-- ret=%d\n",len );
        break;
#endif
    case RTSP_LOWER_TRANSPORT_UDP:
    case RTSP_LOWER_TRANSPORT_UDP_MULTICAST:
...
redo:
    ...
    if (rt->transport == RTSP_TRANSPORT_RDT) {
        ...
        //av_log(s, AV_LOG_DEBUG, "ff_rtp_parse_packet--> \n");
        ret = ff_rtp_parse_packet(rtsp_st->transport_priv, pkt, &rt->recvbuf, len);
        //av_printf_byte("", rt->recvbuf, 16);
        //av_log(s, AV_LOG_DEBUG, "ff_rtp_parse_packet<-- ret=%d\n",ret);
        if (rtsp_st->feedback) {
            AVIOContext *pb = NULL;
        ...
        //av_log(s, AV_LOG_DEBUG, "avpriv_mpegts_parse_packet--> \n");
        ret = avpriv_mpegts_parse_packet(rt->ts, pkt, rt->recvbuf, len);
        //av_log(s, AV_LOG_DEBUG, "avpriv_mpegts_parse_packet<-- ret=%d\n",ret);
        if (ret >= 0) {
            if (ret < len) {
                rt->recvbuf_len = len;
                rt->recvbuf_pos = ret;
