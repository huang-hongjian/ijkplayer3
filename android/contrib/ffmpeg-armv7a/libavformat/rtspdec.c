...
static int rtsp_get_seek_utc_time(int64_t seek_timestamp,char* seek_time)
{
    if(!seek_time || seek_timestamp <= 0)
        return -1;
    time_t t;
    struct tm * lt;
    //struct tm tt;
    t = av_gettime()/1000000;
    //t=t - 8* 3600 - seek_timestamp / AV_TIME_BASE;//UTC time
    t=t  - seek_timestamp / AV_TIME_BASE;
    av_log(NULL, AV_LOG_DEBUG, "seek_time=%lld\n", t);
    lt = localtime (&t);

    if(!lt)
        return -1;
    /*
    tt.tm_year = lt->tm_year+1900;
    tt.tm_mon = lt->tm_mon+1;
    tt.tm_mday = lt->tm_mday;
    tt.tm_hour = lt->tm_hour;
    tt.tm_min = lt->tm_min;
    tt.tm_sec =  lt->tm_sec;
    */
    //strftime(seek_time, sizeof(seek_time), "%Y%m%dT%H:%M:%S.00Z", lt);
    sprintf(seek_time,"%04d%02d%02dT%02d%02d%02d.00Z",
        lt->tm_year+1900,lt->tm_mon+1,lt->tm_mday,
        lt->tm_hour,lt->tm_min,lt->tm_sec);
    av_log(NULL, AV_LOG_DEBUG, "seek_time=%s\n", seek_time);
    return 0;
}
static int rtsp_read_play(AVFormatContext *s)
{
    ...
    if (!(rt->server_type == RTSP_SERVER_REAL && rt->need_subscription)) {
        ...
        if (rt->state == RTSP_STATE_PAUSED) {
            ...
            if(title && title->value && !strcmp(title->value, "live"))
            {
                char seek_time[50] = {0};
                if(rtsp_get_seek_utc_time(rt->seek_timestamp,seek_time) ==0)
                    snprintf(cmd, sizeof(cmd),"Range: clock=%s\r\n",seek_time);
                else
                    snprintf(cmd, sizeof(cmd),"Range: clock=end\r\n");
                
                av_log(NULL,AV_LOG_DEBUG,"rtsp_read_play seek_timestamp=%lld",rt->seek_timestamp);
                //snprintf(cmd, sizeof(cmd),"Range: clock=20190124T055600Z\r\n");
            }
            else
            {
...
retry:
    ...
    //av_log(NULL,AV_LOG_DEBUG,"ff_rtsp_fetch_packet -->");
    ret = ff_rtsp_fetch_packet(s, pkt);
    //av_log(NULL,AV_LOG_DEBUG,"ff_rtsp_fetch_packet<-- ret=%d",ret);
    if (ret < 0) {
                          ...
                          int64_t timestamp, int flags)
{
    ...
    av_log(NULL,AV_LOG_DEBUG,"%s %d seekpos=%lld timestamp=%lld start_time=%lld",__FUNCTION__,__LINE__,rt->seek_timestamp,timestamp,s->start_time);
    if(s->start_time >0 && s->start_time != AV_NOPTS_VALUE)
        rt->seek_timestamp = rt->seek_timestamp - s->start_time;
    switch(rt->state) {
    default:
