...
static const AVCodec *find_probe_decoder(AVFormatContext *s, const AVStream *st, enum AVCodecID codec_id)
{
    ...
    {
        AVCodec* codec = avcodec_find_decoder_by_name("h264");//h264_mediacodec
        if(!codec)
            av_log(NULL,AV_LOG_DEBUG,"hongjian: codec==null");
        else
            av_log(NULL,AV_LOG_DEBUG,"hongjian: codec==%s",codec->name);
        return codec;
    }
...
int ff_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        pkt->size = 0;
        av_init_packet(pkt);
        //av_log(NULL,AV_LOG_DEBUG,"ff_read_packet--> ret = %d",ret);
...
int av_read_frame(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        //av_log(NULL,AV_LOG_DEBUG,"read_frame_internal ret = %d",ret);
        if (ret < 0) {
            if (pktl && ret != AVERROR(EAGAIN)) {
                eof = 1;
                continue;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    av_log(NULL,AV_LOG_DEBUG,"avformat_find_stream_info----->1  time=%lld",av_gettime());
    ...
    for (;;) {
        ...
        if (ret < 0) {
            /* EOF or error*/
            eof_reached = 1;
            av_log(NULL, AV_LOG_DEBUG,"avformat_find_stream_info eof_reached=1");
            break;
        }
    ...
    av_log(NULL,AV_LOG_DEBUG,"avformat_find_stream_info----->2  time=%lld",av_gettime());
    if (eof_reached) {
        int stream_index;
    ...
    av_log(NULL,AV_LOG_DEBUG,"avformat_find_stream_info----->3  time=%lld",av_gettime());
    for (i = 0; i < ic->nb_streams; i++) {
        st = ic->streams[i];
        avctx = st->internal->avctx;
        if (avctx->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    av_log(NULL,AV_LOG_DEBUG,"avformat_find_stream_info----->4  time=%lld",av_gettime());
    ...
    av_log(NULL,AV_LOG_DEBUG,"avformat_find_stream_info----->5  time=%lld",av_gettime());
...
AVProgram *av_new_program(AVFormatContext *ac, int id)
{
    ...
    av_log(ac, AV_LOG_DEBUG, "hongjian:   new_program: id=0x%04x nb_programs:%d\n", id,ac->nb_programs);
...
static int try_decode_framex(AVFormatContext *s, AVStream *st, AVPacket *avpkt,
            AVDictionary **options)
        {
            const AVCodec *codec;
            int got_picture = 1, ret = 0;
...
fail:
        ...
        static int ff_alloc_extradatax(AVCodecContext *avctx, int size)
        {
            int ret;
        ...
        static AVPacket *add_to_pktbufx(AVPacketList **packet_buffer, AVPacket *pkt,
        ...
        static void free_packet_bufferx(AVPacketList **pkt_buf, AVPacketList **pkt_buf_end)
        {
            while (*pkt_buf) {
                AVPacketList *pktl = *pkt_buf;
                *pkt_buf = pktl->next;
        ...
        static int has_codec_parametersx(AVStream *st, const char **errmsg_ptr,int keypacket)
        {
...
#define FAIL(errmsg) do {                                         \
    ...
    *errmsg_ptr = errmsg;                                 \
    return 0;                                                 \
        ...
        static    const AVCodec *find_decoderx(AVFormatContext *s, AVStream *st, enum AVCodecID codec_id)
        {
            if (st->codec->codec)
            return st->codec->codec;
        ...
        static int alloc_and_copy_intra_matrixx(AVCodecContext *dest, const AVCodecContext *src,size_t size ,size_t pad)
        {
            if(src->intra_matrix && size > 0)
            {
                dest->intra_matrix = (uint16_t *)av_malloc(size + pad);
                if(dest->intra_matrix == NULL)
                {
                    goto fail;
                }
        ...
        static int alloc_and_copy_inter_matrixx(AVCodecContext *dest, const AVCodecContext *src,size_t size ,size_t pad)
        {
            if(src->inter_matrix && size > 0)
            {
                dest->inter_matrix = (uint16_t *)av_malloc(size + pad);
                if(dest->inter_matrix == NULL)
                {
                    goto fail;
                }
        ...
        static int alloc_and_copy_rc_overridexx(AVCodecContext *dest, const AVCodecContext *src,size_t size ,size_t pad)
        {
            if(src->rc_override && size > 0)
            {
                dest->rc_override = (RcOverride *)av_malloc(size + pad);
                if(dest->rc_override == NULL)
                {
                    goto fail;
                }
        ...
        static int avcodec_copy_context_privatex(AVCodecContext *dest, const AVCodecContext *src)
        {
            if(!src)
            {
                return -1;
        ...
        /*    if (src->rc_eq) {
            ...
            ret = alloc_and_copy_intra_matrixx(dest, src, 64 * sizeof(int16_t), 0);
            if(ret < 0)
            {
                return ret ;
            }
            ret = alloc_and_copy_inter_matrixx(dest, src, 64 * sizeof(int16_t), 0);
            if(ret < 0)
            {
                return ret ;
            }
            ret = alloc_and_copy_rc_overridexx(dest, src,src->rc_override_count * sizeof(*src->rc_override), 0);
            return ret ;
        }

         int avformatFindStreamInfo(AVFormatContext *ic, AVDictionary **options)
        {
            int i, count, ret = 0, j;
            int64_t read_size;
            AVStream *st;
            ...
            int firstKeyPacket = 0;
            int tryCount = 0;
            int orig_nb_streams = ic->nb_streams;
            int flush_codecs    = ic->probesize > 0;
            int probesize = ic->probesize;
            ...
            if (!strcmp(ic->iformat->name, "flv"))
                max_analyze_duration = 90*AV_TIME_BASE;
            if (!strcmp(ic->iformat->name, "mpeg") || !strcmp(ic->iformat->name, "mpegts"))
                max_analyze_duration = 7*AV_TIME_BASE;
            int is_live = 0;
            if(strstr(ic->filename,"rtp:"))
                is_live = 1;
            for (i = 0; i < ic->nb_streams; i++) {
                const AVCodec *codec;
                AVDictionary *thread_opt = NULL;
                ...
                av_dict_set(options ? &options[i] : &thread_opt, "threads", "1", 0);
                if (!has_codec_parametersx(st, NULL,0) && st->request_probe <= 0) {
            ...
            read_size = 0;
            int saveAudioFlag = 0;
            //shared_ptr<AVPacket> lastAudioPacket[AudioPacketsNum] = {NULL};
            ...
            for (;;) {
                ...
                for (i = 0; i < ic->nb_streams; i++) {
                    st = ic->streams[i];
                    if (!has_codec_parametersx(st, NULL,firstKeyPacket))
                        break;
                ...
                if (!(ic->flags & AVFMT_FLAG_NOBUFFER)&&pkt->size>0) {
                    AVPacket* ret1 = add_to_pktbufx(&ic->internal->packet_buffer, pkt,
                                        &ic->internal->packet_buffer_end);
                    if (ret1 ==NULL )
                        av_log(ic, AV_LOG_DEBUG, "add_to_pktbuf err ");//goto find_stream_info_err;
                }
                
                if(ic->nb_streams == 0){ ret = -1;goto err;}
                 int codeType = ic->streams[pkt->stream_index]->codec->codec_type;
                ...
                if (st->parser && st->parser->parser->split && !st->codec->extradata) {
                    ...
                    if (i > 0 && i < FF_MAX_EXTRADATA_SIZE) {
                        if (ff_alloc_extradatax(st->codec, i))
                            {ret =  AVERROR(ENOMEM);goto err;}
                        memcpy(st->codec->extradata, pkt->data,
                            st->codec->extradata_size);
                    }
                }
                int decodeRet = try_decode_framex(ic, st, pkt,
                    (options && i < orig_nb_streams) ? &options[i] : NULL);
                st->codec_info_nb_frames++;
