...
#define HW_DECODER_PTS_MAX (0x7fffffff) // ((~(unsigned int)0)/2)
...
static int handleH2645(Decoder *d, AVPacket *in, HalPacket *out)
{
    int ret = 0;
    ...
    ret = av_bsf_send_packet(d->hbsf, in);
    if (ret < 0) {
        return -1;
    }

    ret = av_bsf_receive_packet(d->hbsf, &d->pkt_temp);
    if (ret < 0) {
        return -1;
    }

    out->data = d->pkt_temp.data;
    out->len = d->pkt_temp.size;
    return ret;
}

static int get_aac_header(AVCodecContext *avctx, int buflen, char*put)
{
    if(!avctx || !put)
        return -1;
    ...
    bits[0] = 0xff;
    bits[1] = 0xf1;
    bits[2] = 0x40 | (samfreindex<<2) | (channelconfig>>2);
    bits[3] = ((channelconfig&0x3)<<6) | (length >>11);
...
static int handleAAC(VideoState *is,Decoder *d, AVPacket *in, HalPacket *out, char *header)
{
    //av_printf_byte("hongjian", in->data, 4);
    //av_log(NULL,AV_LOG_DEBUG,"hongjian:extradata_size=%d",d->avctx->extradata_size);
    if ((in->data[0] == 0xff && in->data[1] == 0xf1) || d->avctx->extradata_size <= 0) {
        out->data = in->data;
        out->len = in->size;
        return 0;
    } else {
        get_aac_header(d->avctx, in->size, header);
        out->header_len = 7;
        out->data = in->data;
        out->len = in->size;
        out->header = (uint8_t*)header;
    }
    return -1;
}

static int hbsf_init(AVCodecContext *avctx, Decoder *d)
{
    const char *filter_name = NULL;
    switch((int)avctx->codec_id) {
    ...
    if (filter_name != NULL) {
        ...
        if (!d->hbsf) {
            ...
            av_log(NULL, AV_LOG_ERROR, "hbsf_init line=%d bsf=%p", __LINE__, filter_name);
            if (bsf) {
                ret = av_bsf_alloc(bsf, &d->hbsf);
                av_log(NULL, AV_LOG_ERROR, "hbsf_init line=%d", __LINE__);
                ret = avcodec_parameters_from_context(d->hbsf->par_in, avctx);
                if (ret < 0)
                    return ret;

                d->hbsf->time_base_in = avctx->time_base;
                ret = av_bsf_init(d->hbsf);
...
static int hw_write_es_data_hisi( HalPacket *pPkt)
{
    if(!pPkt)
        return 0;
    if(pPkt->type == PLAYER_STREAMTYPE_VIDEO || pPkt->type == PLAYER_STREAMTYPE_AUDIO) {

        uint8_t *pBuffer = NULL;
        int64_t big_number =   HW_DECODER_PTS_MAX;
        if(pPkt->pts > big_number)
            pPkt->pts = pPkt->pts%big_number;
        int ret = 0;
        if(count %100 == 0)
            av_log(NULL,AV_LOG_DEBUG,"%s %d pPkt->pts=%lld",__FUNCTION__,__LINE__,pPkt->pts);
        // av_log(NULL,AV_LOG_DEBUG,"%s %d len=%d header_len=%d type=%d",__FUNCTION__,__LINE__,pPkt->len,pPkt->header_len,pPkt->type);
        uint32_t nSize = pPkt->len + pPkt->header_len;
        ret = HJ_GetWriteBuffer(pPkt->type, &pBuffer, &nSize);

        if(ret == 0) {
            if(pPkt->header_len >0)
                memcpy(pBuffer,pPkt->header,pPkt->header_len);
            if(pPkt->len > 0)
                memcpy(pBuffer+pPkt->header_len,pPkt->data,pPkt->len);
            ret = HJ_WriteData(pPkt->type, pBuffer, nSize,  pPkt->pts);
            //if(ret)
            	av_log(NULL,AV_LOG_DEBUG,"%s %d ret=%d nSize=%d",__FUNCTION__,__LINE__,ret,nSize);
...
static int hw_write_es_data_amlogic( HalPacket *pPkt)
{
    if(!pPkt)
        return 0;
    if(pPkt->type == PLAYER_STREAMTYPE_VIDEO || pPkt->type == PLAYER_STREAMTYPE_AUDIO) {
        ...
        //av_log(NULL,AV_LOG_DEBUG,"%s %d len=%d header_len=%d type=%d",__FUNCTION__,__LINE__,pPkt->len,pPkt->header_len,pPkt->type);
        uint32_t nSize = pPkt->len + pPkt->header_len;
        ret = HJ_GetWriteBuffer(pPkt->type, &pBuffer, &pBufferLen);
        if(!pBuffer)
            return 0;
        if(pBufferLen >= nSize) {
            if(pPkt->header_len >0)
            ...
            if(pPkt->header_len >0) {
                memcpy(pBuffer,pPkt->header,pPkt->header_len);
                ret = HJ_WriteData(pPkt->type, pBuffer, nSize,  pPkt->pts);
                nSize -= pPkt->header_len;
                //av_log(NULL,AV_LOG_DEBUG,"1111111 %s %d ret=%d nSize=%d",__FUNCTION__,__LINE__,ret,nSize);
            }
            if(pPkt->len > 0) {
                int size = (512*1024);
                int num = nSize/size;
                ...
                for(i=0; i<num; i++) {
                    ...
                    memcpy(pBuffer,pPkt->data,size);
                    ret = HJ_WriteData(pPkt->type, pBuffer, size,  pPkt->pts);
                    nSize -= size;
                    //av_log(NULL,AV_LOG_DEBUG,"1111111 %s %d ret=%d nSize=%d",__FUNCTION__,__LINE__,ret,nSize);
                }
                memcpy(pBuffer,pPkt->data,nSize);
                ret = HJ_WriteData(pPkt->type, pBuffer, nSize,  pPkt->pts);
                //av_log(NULL,AV_LOG_DEBUG,"1111111 %s %d ret=%d nSize=%d",__FUNCTION__,__LINE__,ret,nSize);
            }
...
static void hal_event_cb(void *handler, PLAYER_EVENT_E evt, uint32_t param1, uint32_t param2)
{
    FFPlayer*  ffp = (FFPlayer*)handler;
    av_log(NULL,AV_LOG_DEBUG,"%s %d %d",__FUNCTION__,__LINE__,evt);
    if(!ffp)
        return;
    switch((int)evt) {
    case PLAYER_EVENT_FIRST_PTS: {
        if(!ffp->report_first_frame) {
            ffp_notify_msg3(ffp, FFP_MSG_VIDEO_FIRST_FRAME, param1, param2);
            ffp->report_first_frame = 1;
        } else if(ffp->is->hw_decoder_seek) {
            ffp_notify_msg3(ffp, FFP_MSG_SEEK_AFTER_FIRST_FRAME, param1, param2);
            ffp->is->hw_decoder_seek =0;
            ffp->is->hw_seek_pos = -1;
            ffp->is->hw_buffering_end_time = av_gettime();
        }

    }
    break;
    case PLAYER_EVENT_BLURREDSCREEN_START:
        ffp_notify_msg3(ffp, FFP_MSG_BLURREDSCREEN_START, param1, param2);
        break;
    case PLAYER_EVENT_BLURREDSCREEN_END:
        ffp_notify_msg3(ffp, FFP_MSG_BLURREDSCREEN_END, param1, param2);
        break;
    case PLAYER_EVENT_UNDERFLOW_START:
        ffp_notify_msg3(ffp, FFP_MSG_UNDERFLOW_START, param1, param2);
        break;
    case PLAYER_EVENT_UNDERFLOW_END:
        ffp_notify_msg3(ffp, FFP_MSG_UNDERFLOW_END, param1, param2);
        break;
    default:
...
int ffp_get_hardware(FFPlayer *ffp)
{
    if(!ffp)
        return -1;

    if(ffp->avplayer_enable) {
...
static int64_t packet_queue_get_index(PacketQueue *q)
{
    MyAVPacketList *pkt1;
    AVPacket *pkt;
    ...
    if (pkt1) {
        ...
        if (pkt) {
            ...
            idx = pkt->index;
            //idx = pkt->pts;
            //av_log(NULL, AV_LOG_ERROR, "packet_queue_get_index pkt1 is idx=%d, streamidx=%d\n", idx, pkt->stream_index);
        }
    } else {
        //av_log(NULL, AV_LOG_ERROR, "packet_queue_get_index pkt1 is null\n");
    }
    SDL_UnlockMutex(q->mutex);
    return idx;
}
...
static int packet_queue_put_nullpacket(PacketQueue *q, int stream_index,int64_t index)
{
...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block, int *serial)
{
    MyAVPacketList *pkt1;
    int ret;
    //      av_log(NULL,AV_LOG_DEBUG,"%s %d  ",__FUNCTION__,__LINE__);
    SDL_LockMutex(q->mutex);
    //      av_log(NULL,AV_LOG_DEBUG,"%s %d  ",__FUNCTION__,__LINE__);
    for (;;) {
        if (q->abort_request) {
...
static int check_buffering_report(FFPlayer *ffp,int report_falg)
{
    assert(ffp);
    VideoState *is = ffp->is;
    if(is && is->hw_decode_pts != -1LL && is->hw_not_decode_pts !=-1LL) {
        is->hw_decode_cache_time = is->hw_not_decode_pts - is->hw_decode_pts;
    } else if(is)
        is->hw_decode_cache_time = -1LL;

    if(!is)
        ffp_toggle_buffering(ffp, 1);
    if(is && is->hw_decode_cache_time == -1LL)
        ffp_toggle_buffering(ffp, 1);
    else if(is && is->hw_decode_cache_time < HW_DECODER_CACHE_TIME) {
        
        av_log(NULL,AV_LOG_DEBUG,"%s %d  ffp_toggle_buffering(ffp, 1) -------*_*------->hw_decode_cache_time=%lld",__FUNCTION__,__LINE__,is->hw_decode_cache_time);

        if(report_falg && !ffp->is->hw_decoder_seek) {
            av_log(NULL,AV_LOG_DEBUG,"%s %d  ffp_toggle_buffering(ffp, 1) -------------->",__FUNCTION__,__LINE__);
            ffp_toggle_buffering(ffp, 1);
        }
...
static int packet_queue_get_or_buffering(FFPlayer *ffp, PacketQueue *q, AVPacket *pkt, int *serial, int *finished)
{
    ...
    if (!ffp->packet_buffering) {
        ...
        //   av_log(NULL,AV_LOG_DEBUG,"%s %d packet_queue_get ret =%d  packet_buffering=%d",__FUNCTION__,__LINE__ ,ret ,ffp->packet_buffering);
        return ret;
    ...
    while (1) {
        ...
        //  av_log(NULL,AV_LOG_DEBUG,"%s %d packet_queue_get new_packet=%d q->is_buffer_indicator=%d",__FUNCTION__,__LINE__ ,new_packet,q->is_buffer_indicator);
        if (new_packet < 0)
            return -1;
            ...
            if (q->is_buffer_indicator && !*finished) {
                if(ffp->avplayer_enable) {
                    ...
                    //av_log(NULL,AV_LOG_DEBUG,"%s %d  ffp_toggle_buffering(ffp, 1) hw_decode_pts=%lld hw_not_decode_pts=%lld hw_decode_cache_time=%lld",__FUNCTION__,__LINE__,
                    //           is->hw_decode_pts,is->hw_not_decode_pts,is->hw_decode_cache_time);
                } else
                    ffp_toggle_buffering(ffp, 1);
            }
            //   av_log(NULL,AV_LOG_DEBUG,"%s %d packet_queue_get new_packet=%d------------->",__FUNCTION__,__LINE__,new_packet);
    ...
    // av_log(NULL,AV_LOG_DEBUG,"ffp_toggle_buffering ret ====1111111111");
    return 1;
}
        ...
        do {
            ...
            if (d->packet_pending) {
                ...
                if (packet_queue_get_or_buffering(ffp, d->queue, &pkt, &d->pkt_serial, &d->finished) < 0)
                    return -1;
                //av_log(NULL,AV_LOG_DEBUG,"%s %d audio pkt size=%d",__FUNCTION__,__LINE__,pkt.size);
            }
...
static void stream_component_close(FFPlayer *ffp, int stream_index)
{
    ...
    ic->streams[stream_index]->discard = AVDISCARD_NONE;
    switch (codecpar->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
        is->audio_st = NULL;
        is->audio_stream = -1;
...
static void stream_close(FFPlayer *ffp)
{
    ...
    if(ffp->avplayer_enable && is->hardware == AMLOGIC) {
        SDL_WaitThread(is->hw_tid, NULL);
        is->hw_tid = NULL;
    }
...
static int update_hw_not_decode_pts(VideoState *is ,int64_t pts)
{
    if(!is && pts != 0xFFFFFFFFU)
        return -1;
    is->hw_not_decode_pts =pts/90;
    is->big_number_pts =  pts/HW_DECODER_PTS_MAX;
    // if(is->big_number_pts )
    if(count %100 == 0)
        av_log(NULL,AV_LOG_DEBUG,"update_hw_not_decode pts=%lld",pts);
    return 0;
}
static int hw_thread(void *arg)
{
    FFPlayer *ffp = arg;
    VideoState *is = ffp->is;
    Decoder *ad = &is->auddec;
    Decoder *vd = &is->viddec;
    Decoder *d = NULL;
    int type = 0;
    int ret = 0;
    int check_empty_num = 0;
    int audio_index = 0;
    int video_index = 0;
    HalPacket halpkt;
    ...
    char aac_header[7];
    //is->sync_pts = 0LL;
    //av_log(NULL, AV_LOG_ERROR, "hw_thread-->");
    for (;;) {

        if(is->abort_request) {

            av_log(NULL, AV_LOG_ERROR, "hw_thread flag00=%d\n", is->abort_request);
            break;
        }
        if (vd->queue) {
            if (vd->queue->abort_request) {
                break;
            }
        }

        if (ad->queue) {
            if (ad->queue->abort_request) {
                break;
            }
        }

        if (is->paused || is->seek_req) {
            av_usleep (1000*10);
            continue;
        }
        ...
        if (vd->queue && vd->queue->abort_request == 0)
            video_index = packet_queue_get_index(vd->queue);
        if (ad->queue && ad->queue->abort_request == 0)
            audio_index = packet_queue_get_index(ad->queue);

        // av_log(NULL, AV_LOG_DEBUG,"%s %d audio_index=%d video_index=%d",__FUNCTION__,__LINE__,audio_index,video_index);
        // If the decode buffer is empty, report "FFP_MSG_COMPLETED" event
        if (audio_index == -1 && video_index == -1) {
            //ffp_private_check_compelete(ffp, &check_empty_num);
            if (check_empty_num > 5) {
        ...
        if (video_index != -1 && audio_index != -1) {
            if (video_index < audio_index ) {
                d = vd;
                type = PLAYER_STREAMTYPE_VIDEO;
            } else {
                d = ad;
                type = PLAYER_STREAMTYPE_AUDIO;
            }
        ...
        if (d == NULL || d->queue == NULL) {
            continue;
        }
        //av_log(NULL, AV_LOG_DEBUG,"%s %d codec_id=%x type=%d",__FUNCTION__,__LINE__,d->avctx->codec_id, type);
        if (audio_index != 0 ) {
            //is->queue_index_r++;
        }
        ...
        if (pkt.data == flush_pkt.data) {
            avcodec_flush_buffers(d->avctx);
            d->finished = 0;
            d->next_pts = 0;//d->start_pts;
            d->next_pts_tb = d->start_pts_tb;
        }
        if(type == PLAYER_STREAMTYPE_AUDIO) {
            // convert the timebase
            time_base =(is->audio_st != NULL ? &(is->audio_st->time_base) : NULL) ;
            if (time_base) {
                pts = pkt.pts > 0 ? av_rescale_q(pkt.pts, *time_base, (AVRational) {
                    1, 90000
                }) :
                    (pkt.dts > 0 ? av_rescale_q(pkt.dts, *time_base, (AVRational) {
                    1, 90000
                }):0xFFFFFFFFU);
            } else {
                pts = pkt.pts > 0 ? pkt.pts : (pkt.dts > 0 ? pkt.dts : 0xFFFFFFFFU);
            }
        } else if(type == PLAYER_STREAMTYPE_VIDEO) {
            time_base =(is->video_st != NULL ? &(is->video_st->time_base) : NULL) ;
            if (time_base) {
                pts = pkt.pts > 0 ? av_rescale_q(pkt.pts, *time_base, (AVRational) {
                    1, 90000
                }) :
                ...
                //av_log(NULL, AV_LOG_DEBUG,"%s %d pts=%lld pkt.pts=%lld num=%d den=%d",__FUNCTION__,__LINE__,pts,pkt.pts,time_base->num,time_base->den);
...
//av_log(NULL, AV_LOG_DEBUG,"%s %d codec_id=%x type=%d",__FUNCTION__,__LINE__,d->avctx->codec_id, type);
        //ffp_private_push_data_info(is, type, pkt.size, pts);
        //av_log(NULL,AV_LOG_DEBUG,"hongjian: codec_id=%x",d->avctx->codec_id);
        switch(d->avctx->codec_id) {

        case AV_CODEC_ID_AC3:
        case AV_CODEC_ID_AAC:
            //av_log(NULL,AV_LOG_DEBUG,"hongjian: handleAAC----->");
            handleAAC(is,d, &pkt, &halpkt, aac_header);
            break;
        case AV_CODEC_ID_H264:
        case AV_CODEC_ID_HEVC:
            handleH2645(d, &pkt, &halpkt);
            break;

        case AV_CODEC_ID_MPEG4:
            if (d->avctx->extradata_size > 0) {
                halpkt.header_len = d->avctx->extradata_size;
                halpkt.header = d->avctx->extradata;
            }
            break;
        default:
            halpkt.data = pkt.data;
            halpkt.len = pkt.size;
            break;
        }
...
//av_log(NULL, AV_LOG_DEBUG,"%s %d codec_id=%x type=%d",__FUNCTION__,__LINE__,d->avctx->codec_id, type);
        halpkt.type = type;
        halpkt.pts = pts;
        update_hw_not_decode_pts(is,pts);
        //is->sync_pts = pts;
        //av_printf_byte("hongjian:",halpkt.data,16);
        //av_log(NULL,AV_LOG_DEBUG,"%s %d audio pkt size=%d",__FUNCTION__,__LINE__,halpkt.len);
        //av_log(NULL, AV_LOG_DEBUG,"%s %d pts=%lld pkt.pts=%lld num=%d den=%d",__FUNCTION__,__LINE__,pts,pkt.pts,time_base->num,time_base->den);
        while(1) {
            if(is->abort_request)
                break;
            if(is->hardware == AMLOGIC)
                ret = hw_write_es_data_amlogic(&halpkt);
            else
                ret = hw_write_es_data_hisi(&halpkt);
            //if(ret)av_log(NULL,AV_LOG_DEBUG,"%s %d hw_write_es_data aud ret=%d",__FUNCTION__,__LINE__,ret);
            if (d->queue) {
                if (d->queue->abort_request || ret == 0) {
                    break;
                }
            }
            //av_usleep(5*1000);
...
static int hw_audio_thread(void *arg)
{
    FFPlayer *ffp = arg;
    VideoState *is = ffp->is;
    Decoder *ad = &is->auddec;
    ...
    int64_t pts;
    char aac_header[7];
    int have_video = vd->avctx->codec_id != AV_CODEC_ID_NONE ? 1 : 0;
    int is_live =(is->stream_type == AV_STREAM_LIVE)?1:0;
    int report_flag = 0;
    int64_t start_time = 0LL;
    int64_t end_time = 0LL;
    long completed_diff = 0;
    ...
    for (;;) {
        ...
        if ((ad->queue && ad->queue->abort_request))
            break;

        if (is->paused || is->seek_req) {
           // av_log(NULL, AV_LOG_ERROR, "is->paused=1");
            av_usleep (1000*10);
            continue;
        }
        //av_log(NULL,AV_LOG_DEBUG,"%s %d",__FUNCTION__,__LINE__);
        ...
        if (audio_index == -1 && video_index == -1) {
            ...
            if(is_live==0) {
                ...
                if(is->is_completed == 0 && check_empty_num > CHECK_EMPTY_BUFFER_MIN_NUM) {
                    //av_log(NULL, AV_LOG_DEBUG,"%s %d FFP_MSG_COMPLETED ---> pos=%ld duration=%ld completed_count=%d",__FUNCTION__,__LINE__,pos,duration,completed_count);
                    if(duration -pos == completed_diff)
                        completed_count++;
                    else
                        completed_count = 0;
                    completed_diff = duration -pos;
                    if(pos > 0L && duration > 0L && completed_diff > 0  && completed_diff < COMPLETED_DELAY) { //100 ms
                        is->is_completed = 1;
                        av_log(NULL, AV_LOG_DEBUG,"%s %d FFP_MSG_COMPLETED pos=%ld duration=%ld",__FUNCTION__,__LINE__,pos,duration);
                        ffp_notify_msg1(ffp, FFP_MSG_COMPLETED);
                        break;
                    }

                    else if(completed_diff > 0  && completed_diff < COMPLETED_RANGE && completed_count == COMPLETED_DELAY_COUNT) {
                        is->is_completed = 1;
                        av_log(NULL, AV_LOG_DEBUG,"%s %d FFP_MSG_COMPLETED2 pos=%ld duration=%ld",__FUNCTION__,__LINE__,pos,duration);
                        ffp_notify_msg1(ffp, FFP_MSG_COMPLETED);
                        break;
                    }
                } else if(is->is_completed )
                    break;

            }
            //av_log(NULL,AV_LOG_DEBUG,"%s %d is_completed=%d  pos=%ld duration=%ld ",__FUNCTION__,__LINE__,is->is_completed,pos,duration );
            if(is_live==0 && is->is_completed ==0 && pos > 0 && duration > 0&& duration- pos > BUFFERING_STATUS_RANGE) {
                //av_log(NULL,AV_LOG_DEBUG,"%s %d 22222222  ffp_toggle_buffering(ffp, 1) ",__FUNCTION__,__LINE__ );
                end_time = av_gettime();
                if(end_time != 0LL && start_time != 0LL ) {
                    if( end_time - start_time >= BUFFERING_START_DELAY
                        && (is&&end_time- is->hw_buffering_end_time > BUFFERING_END_AFTER_REPORT_DELAY)) {
                        //av_log(NULL,AV_LOG_DEBUG,"ffp_toggle_buffering 1111111111");
                        check_buffering_report(ffp,1);
                        start_time = 0L;
                        end_time = 0L;
                ...
                int is_report = check_buffering_report(ffp,0);
                //av_log(NULL,AV_LOG_DEBUG,"%s %d  is_report=%d ffp_toggle_buffering(ffp, 1) hw_decode_pts=%lld hw_not_decode_pts=%lld hw_decode_cache_time=%lld",__FUNCTION__,__LINE__,
                //           is_report,is->hw_decode_pts,is->hw_not_decode_pts,is->hw_decode_cache_time);
                if(is_report == 1) {
                    av_usleep(5000);
                    start_time =av_gettime();
                    //check_buffering_report(ffp,is_report);
                    continue;
                }

            }
            //       av_log(NULL, AV_LOG_DEBUG,"%s %d audio_index=%d video_index=%d check_empty_num=%d",__FUNCTION__,__LINE__,audio_index,video_index,check_empty_num);
        ...
        //av_log(NULL,AV_LOG_DEBUG,"%s %d audio_index=%d video_index=%d",__FUNCTION__,__LINE__,audio_index,video_index);
        else if (audio_index != -1 && video_index != -1  ) {
            check_empty_num = 0;
        ...
        memset(&pkt, 0, sizeof(AVPacket));
        if (packet_queue_get_or_buffering(ffp, d->queue, &pkt, &d->pkt_serial, &d->finished) < 0) {
            //  av_log(NULL,AV_LOG_DEBUG,"%s %d  packet_queue_get_or_buffering ret<0",__FUNCTION__,__LINE__);
    ...
    av_log(NULL,AV_LOG_DEBUG,"%s %d ************exit",__FUNCTION__,__LINE__);
    return 0;
}

static int hw_video_thread(void *arg)
{
    FFPlayer *ffp = arg;
    VideoState *is = ffp->is;
    Decoder *vd = &is->viddec;
    ...
    AVPacket pkt;
    int64_t pts;
    int have_audio = ad->avctx->codec_id != AV_CODEC_ID_NONE ? 1 : 0;
    int is_live = (is->stream_type == AV_STREAM_LIVE)?1:0;;
    int report_flag = 0;
    int64_t start_time = 0LL;
    int64_t end_time = 0LL;
    ...
    for (;;) {
        ...
        if(is->abort_request) {
            ...
            av_log(NULL, AV_LOG_ERROR, "hw_video_thread flag00=%d\n", is->abort_request);
            break;
        }
        if ((vd->queue && vd->queue->abort_request))
            break;
        //av_log(NULL, AV_LOG_ERROR, "hw_thread is->paused=%d is->seek_req=%d",is->paused,is->seek_req);
        if (is->paused || is->seek_req) {
            av_usleep (1000*10);
            continue;
        }
        ...
        if (audio_index == -1 && video_index == -1) {
            ...
            if(is_live==0) {
                ...
                if(is->is_completed == 0 && pos >0 && duration >0 && check_empty_num > CHECK_EMPTY_BUFFER_MIN_NUM) {
                    //av_log(NULL, AV_LOG_DEBUG,"%s %d FFP_MSG_COMPLETED ---> pos=%ld duration=%ld completed_count=%d",__FUNCTION__,__LINE__,pos,duration,completed_count);
                    if(duration -pos == completed_diff)
                        completed_count++;
                    else
                    ...
                    else if(completed_diff > 0  && duration- pos < COMPLETED_RANGE && completed_count == COMPLETED_DELAY_COUNT) {
                        is->is_completed = 1;
                        av_log(NULL, AV_LOG_DEBUG,"%s %d FFP_MSG_COMPLETED2 pos=%ld duration=%ld",__FUNCTION__,__LINE__,pos,duration);
                        ffp_notify_msg1(ffp, FFP_MSG_COMPLETED);
                        break;
...
int get_hal_videodec(enum AVCodecID codec,VIDEO_FORMAT_E* hal_codec)
{
    switch(codec) {
    case AV_CODEC_ID_MPEG1VIDEO:
        *hal_codec = VIDEO_FORMAT_MPEG1;
        break;
    case AV_CODEC_ID_MPEG2VIDEO:
        *hal_codec = VIDEO_FORMAT_MPEG2;
        break;
    case AV_CODEC_ID_MPEG4:
        *hal_codec = VIDEO_FORMAT_MPEG4;
        break;
    case AV_CODEC_ID_H264:
        *hal_codec = VIDEO_FORMAT_H264;

        break;
    case AV_CODEC_ID_HEVC:
        *hal_codec = VIDEO_FORMAT_H265;
        break;
    default:
        *hal_codec = VIDEO_FORMAT_UNKNOWN;

    }
    return 0;
}
int get_hal_audiodec(enum AVCodecID codec,AUDIO_FORMAT_E* hal_codec)
{
    switch(codec) {
    case AV_CODEC_ID_MP2:
    case AV_CODEC_ID_MP3:
        *hal_codec = AUDIO_FORMAT_MPEG;
        break;
    case AV_CODEC_ID_AAC:
    case AV_CODEC_ID_AAC_LATM:
        *hal_codec = AUDIO_FORMAT_AAC;

        break;
    case AV_CODEC_ID_AC3:
        *hal_codec = AUDIO_FORMAT_AC3;
        break;
    case AV_CODEC_ID_EAC3:
        *hal_codec = AUDIO_FORMAT_AC3PLUS;
        break;
    default:
        *hal_codec = AUDIO_FORMAT_UNKNOWN;
...
static int get_video_framerate(AVStream* vstream)
{
    AVStream *vStream = vstream;
    int framerate =0;
    if(vStream->avg_frame_rate.den == 0) {
        framerate = 0;
    } else {
        int frame_rate_temp = (vStream->avg_frame_rate.num)/(vStream->avg_frame_rate.den);

        int mod_rate = (vStream->avg_frame_rate.num)%(vStream->avg_frame_rate.den);
        if(vStream->avg_frame_rate.den <= (mod_rate * 2)) {
            framerate= frame_rate_temp + 1;
        } else {
            framerate = frame_rate_temp;
        }
    }
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    av_log(NULL,AV_LOG_DEBUG,"%s %d stream_index=%d ic->nb_streams=%d",__FUNCTION__,__LINE__,stream_index,ic->nb_streams);
    if (stream_index < 0 || stream_index >= ic->nb_streams)
        return -1;
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if(ffp->avplayer_enable) {
            ...
            AudioPara.pid = 0xffff;//ES_STREAM_PID

            AudioPara.bit_per_sample = 0;
            AudioPara.nChannels = nb_channels;
            AudioPara.nSampleRate = sample_rate;
            AudioPara.pExtraData = NULL;
            AudioPara.nExtraSize = 0;
            AUDIO_FORMAT_E adec ;
            get_hal_audiodec(avctx->codec_id,&adec);
            av_log(NULL,AV_LOG_DEBUG,"%s %d codec_id=0x%x haladec=%x",__FUNCTION__,__LINE__,avctx->codec_id,adec);
            AudioPara.aFmt = adec;
            HJ_InitAudio(&AudioPara);
            is->audio_stream = stream_index;
            is->audio_st = ic->streams[stream_index];
            if(ffp->avplayer_enable)
            {
                if(ffp->is && ffp->is->hardware == AMLOGIC)
                    ffp_set_audio_codec_info(ffp, HICODEC_MODULE_NAME, avcodec_get_name(avctx->codec_id));
                else if(ffp->is &&ffp->is->hardware == HISI)
                    ffp_set_audio_codec_info(ffp, HICODEC_MODULE_NAME, avcodec_get_name(avctx->codec_id));
                else
...
//            ffp_set_audio_codec_info(ffp, AVCODEC_MODULE_NAME, avcodec_get_name(avctx->codec_id));
            ...
            if (avctx->extradata_size > 4) {
                av_log(NULL, AV_LOG_ERROR, "auddec extra size = %d, 0~3=%x, %x, %x, %x", avctx->extradata_size
                       , avctx->extradata[0], avctx->extradata[1], avctx->extradata[2], avctx->extradata[3]);
            }

            hbsf_init(avctx, &is->auddec);
            if(0&& is->hardware == AMLOGIC ) {
                packet_queue_start(is->auddec.queue);
            } else {
                if ((ret = decoder_start(&is->auddec, hw_audio_thread, ffp, "hw_audio_dec")) < 0)
                    goto out;
            }
            break;
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if(ffp->avplayer_enable) {
            ...
            VideoPara.pid = 0xffff;//ES_STREAM_PID
            VideoPara.nVideoWidth = 0;
            VideoPara.nVideoHeight = 0;
            VideoPara.nFrameRate = get_video_framerate(is->video_st) ;
            VideoPara.pExtraData =NULL;
            VideoPara.nExtraSize = 0;
            VIDEO_FORMAT_E vdec ;
            get_hal_videodec(avctx->codec_id,&vdec);
            av_log(NULL,AV_LOG_DEBUG,"%s %d codec_id=0x%x haladec=%x nFrameRate=%d",__FUNCTION__,__LINE__,avctx->codec_id,vdec,VideoPara.nFrameRate);
            VideoPara.vFmt = vdec;
            HJ_InitVideo(&VideoPara);
            ...
            hbsf_init(avctx, &is->viddec);
            if(0&& is->hardware == AMLOGIC ) {
                packet_queue_start(is->viddec.queue);
            } else {
                if ( (ret = decoder_start(&is->viddec, hw_video_thread, ffp, "hw_video_dec")) < 0)
                    goto out;
            }
...
static int read_thread(void *arg)
{
    ...
    int init_ijkmeta = 0;
    char hardware[30];
    int need_change_start_pts_flag = 0;
    int is_seek_change_pts = 0;
    int64_t pts_change_seek_pos = 0;
    int skip_pts_count =0;
    int64_t pts_change_after_current_pts=0;
//    int stream_protocal_type;
    int init_nb_streams = 0;
    if (!wait_mutex) {
        av_log(NULL, AV_LOG_FATAL, "SDL_CreateMutex(): %s\n", SDL_GetError());
        ret = AVERROR(ENOMEM);
        goto fail;
    ...
    memset(hardware, -1, sizeof(hardware));
    if(__system_property_get("ro.hardware", hardware) == 0) {
        av_log(NULL,AV_LOG_DEBUG,"ro.hardware err\n");
    }
    if(strstr(hardware,"amlogic"))
        is->hardware = AMLOGIC;
    ...
    if(is->filename)
    {
        if(strstr(is->filename ,"http://") && strstr(is->filename ,"m3u8") )
            is->protocal_type = AV_STREAM_PROTOCAL_TYPE_HLS;
        else if(strstr(is->filename ,"rtsp://"))
            is->protocal_type = AV_STREAM_PROTOCAL_TYPE_RTSP;
        else if(strstr(is->filename,"udp://"))
            is->protocal_type = AV_STREAM_PROTOCAL_TYPE_UDP;
        else if(strstr(is->filename,"rtp://"))
            is->protocal_type = AV_STREAM_PROTOCAL_TYPE_IGMP;
        else if(strstr(is->filename,"http://"))
            is->protocal_type = AV_STREAM_PROTOCAL_TYPE_HTTP;
    }
    if(is->protocal_type == AV_STREAM_PROTOCAL_TYPE_RTSP &&ic &&  ic->metadata )
    {
        AVDictionaryEntry *title = av_dict_get(ic->metadata, "title",NULL, 0);
        if(title && title->value && strstr(title->value, "live"))
            is->stream_type = AV_STREAM_LIVE;
        if(title && title->value)
           av_log(NULL, AV_LOG_DEBUG,"stream_type=%d protocal_type=%d value=%s",is->stream_type,is->protocal_type, title->value);
   
    }
    else if(is->protocal_type == AV_STREAM_PROTOCAL_TYPE_HLS )
    {
        if(ic && ic->is_hls_vod == 0)
            is->stream_type = AV_STREAM_LIVE;
           av_log(NULL, AV_LOG_DEBUG,"stream_type=%d protocal_type=%d is_hls_vod=%d",is->stream_type,is->protocal_type,ic->is_hls_vod );
    }
    if (err < 0) {
    ...
    is->ic = ic;
    is->hw_decode_pts = -1LL;
    is->hw_not_decode_pts = -1LL;
    is->hw_decode_cache_time = -1LL;
    is->big_number_pts = -1;
    is->hw_decoder_seek = 0;
    is->hw_buffering_end_time = -1LL;
    is->hw_buffering_start_time = -1LL;
    if(ffp->avplayer_enable)
    ...
    if (ffp->find_stream_info) {
        ...
        do {
            ...
            av_log(NULL,AV_LOG_DEBUG,"avformat_find_stream_info--->start=%lld",av_gettime());
            // err = avformatFindStreamInfo(ic, opts);
            err = avformat_find_stream_info(ic, opts);
            av_log(NULL,AV_LOG_DEBUG,"avformat_find_stream_info<---err=%d end=%lld",err,av_gettime());
        } while(0);
    ...
    if(is->stream_type == AV_STREAM_LIVE && is->protocal_type == AV_STREAM_PROTOCAL_TYPE_RTSP)
    {
        if(is->ic->duration <=0)
            is->ic->duration = milliseconds_to_fftime(RTSP_TIMESHIFT_DURATION);
    }
    else if(is->stream_type == AV_STREAM_LIVE && is->protocal_type == AV_STREAM_PROTOCAL_TYPE_HLS)
    {
        if(is->ic->duration <=0)
            is->ic->duration = milliseconds_to_fftime(HLS_TIMESHIFT_DURATION);
    }
    ...
    int h264_stream_number = -1;
    init_nb_streams = ic->nb_streams;
    //av_log(NULL,AV_LOG_DEBUG,"%s %d   avformat_match_stream_specifier-->",__FUNCTION__,__LINE__);
    for (i = 0; i < ic->nb_streams; i++) {
        AVStream *st = ic->streams[i];
        if(st){
            //av_dict_set_int(&st->metadata, "variant_bitrate", bandwidth, 0);
            AVDictionaryEntry *bandwidth = av_dict_get(st->metadata, "variant_bitrate",NULL, 0);
            if(bandwidth && bandwidth->value)
                av_log(NULL,AV_LOG_DEBUG,"hongjian: %s %d   bandwidth=%s",__FUNCTION__,__LINE__,bandwidth->value);
        }

        av_log(NULL,AV_LOG_DEBUG,"hongjian: %s %d   AVStream=%p",__FUNCTION__,__LINE__,st);
    }
    for (i = 0; i < ic->nb_streams; i++) {
        AVStream *st = ic->streams[i];
        ...
        if(codecpar) {
            ...
            if(decoder)
                av_log(NULL,AV_LOG_DEBUG,"hongjian: codec_name=%s",decoder->name);
        }
        ...
        if (type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (codec_id == AV_CODEC_ID_H264) {
                h264_stream_count++;
                h264_stream_number = i;
                if (first_h264_stream < 0)
                    first_h264_stream = i;
            }
        }
        av_log(NULL,AV_LOG_DEBUG,"%s %d %d",__FUNCTION__,__LINE__,type);
    }
    ...
    av_log(NULL,AV_LOG_DEBUG,"%s %d video_disable=%d audio_disable=%d",__FUNCTION__,__LINE__,ffp->video_disable,ffp->audio_disable);
    ...
    if(0&& ffp->avplayer_enable && is->hardware == AMLOGIC) {
        is->hw_tid = SDL_CreateThreadEx(&is->_hw_tid, hw_thread, ffp, "hw_thread_dec");
    }
    ...
    if(ffp->avplayer_enable) {
        ...
        rect.right = 1280;
        rect.top = 0;
        sync.mod = 1;//SYNC_MODE_SLOW
        sync.timeout = 0;
        av_log(NULL,AV_LOG_DEBUG,"%s %d HJ_SetParameter-->6009",__FUNCTION__,__LINE__);
        HJ_SetParameter((int32_t) 6009/*KEY_PARAMETER_VIDEO_POSITION_INFO*/, &rect);
        av_log(NULL,AV_LOG_DEBUG,"%s %d HJ_StartPlay-->",__FUNCTION__,__LINE__);
        HJ_SetEventCB(ffp, hal_event_cb);
        HJ_StartPlay();
        av_log(NULL,AV_LOG_DEBUG,"%s %d HJ_StartPlay<--",__FUNCTION__,__LINE__);
        HJ_SetSurfaceTexture(HJ_GetSurfaceFromApk());
        HJ_SetParameter((int32_t) 1234/*KEY_PARAMETER_SET_SYNC*/, &sync);
        av_log(NULL,AV_LOG_DEBUG,"%s %d HJ_SetParameter-->1234",__FUNCTION__,__LINE__);
...
//      of the seek_pos/seek_rel variables
            is->hw_decoder_seek = 1;
            is->hw_buffering_start_time = av_gettime();
            av_log(NULL,AV_LOG_DEBUG,"%s %d   avformat_seek_file start_time=%lld seek_target=%lld",__FUNCTION__,__LINE__,av_gettime(),seek_target);
            ffp_toggle_buffering(ffp, 1);
            ffp_notify_msg3(ffp, FFP_MSG_BUFFERING_UPDATE, 0, 0);
            pts_change_seek_pos = seek_target;
            ret = avformat_seek_file(is->ic, -1, seek_min, seek_target, seek_max, is->seek_flags);
            av_log(NULL,AV_LOG_DEBUG,"%s %d   avformat_seek_file ret=%d start_time=%lld",__FUNCTION__,__LINE__,ret,av_gettime());
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR,
                       "%s: error while seeking\n", is->ic->filename);
            } else {
                if( is->protocal_type == AV_STREAM_PROTOCAL_TYPE_RTSP)
               {
                    if(is->stream_type == AV_STREAM_LIVE){
                        is->stream_type = AV_STREAM_TIMESHIFT;
                        int64_t seek_timestamp = seek_target;
                        int64_t start_time = ic->start_time;
                        if(seek_timestamp >= 0 && start_time >= 0){
                            is->timeshift_range_live_time = (seek_timestamp - start_time)/1000;
                            av_log(NULL,AV_LOG_DEBUG,"timeshift_range_live_time=%lld seek_timestamp=%lld",is->timeshift_range_live_time,seek_timestamp);
                        }
                    }
                    else{
                        int64_t seek_timestamp = seek_target;
                ...
                if( is->protocal_type == AV_STREAM_PROTOCAL_TYPE_HLS)
               {
                    if(is->stream_type == AV_STREAM_LIVE){
                        is->stream_type = AV_STREAM_TIMESHIFT;
                        int64_t seek_timestamp = seek_target;
        ...
        if (is->queue_attachments_req) {
            if (is->video_st && (is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC)) {
                ...
                if ((ret = av_packet_ref(&copy, &is->video_st->attached_pic)) < 0)
                    goto fail;
                is->queue_index_w++;
                copy.index = is->queue_index_w;
                packet_queue_put(&is->videoq, &copy);
                is->queue_index_w++;
                packet_queue_put_nullpacket(&is->videoq, is->video_stream,is->queue_index_w);
            }
        ...
        if (ffp->infinite_buffer<1 && !is->seek_req &&
            ...
            //av_log(NULL, AV_LOG_DEBUG,"111111111111111111");
            continue;
        }
        // av_log(NULL,AV_LOG_DEBUG,"%s %d",__FUNCTION__,__LINE__);
        if ((!is->paused || completed) &&
            ...
            if (ffp->loop != 1 && (!ffp->loop || --ffp->loop)) {
                ...
                ffp_statistic_l(ffp);
                if (completed) {
                    av_log(ffp, AV_LOG_DEBUG, "ffp_toggle_buffering: eof\n");
                    SDL_LockMutex(wait_mutex);
                    ...
                    toggle_pause(ffp, 1);
                    if (ffp->error) {
                        av_log(ffp, AV_LOG_DEBUG, "ffp_toggle_buffering: error: %d\n", ffp->error);
        ...
        if(count %100 == 0)
            av_log(NULL, AV_LOG_DEBUG,
                  "umplayer av_read_frame  ret=%d streamidx=%d, audidx=%d, vid=%d, pts=%lld, dts=%lld\n"
                  ,ret, pkt->stream_index, is->audio_stream, is->video_stream, pkt->pts, pkt->dts);
        ...
        if (ret < 0) {
            ...
            if (ic->pb && ic->pb->error) {
                ...
                av_log(NULL, AV_LOG_ERROR,"pb_error=%d",pb_error);
            }
            if (ret == AVERROR_EXIT) {
                pb_eof = 1;
            ...
            if (pb_eof) {
                ...
                if (is->audio_stream >= 0) {
                    is->queue_index_w++;
                    packet_queue_put_nullpacket(&is->audioq, is->audio_stream,is->queue_index_w);
                }
                if (is->subtitle_stream >= 0) {
                    is->queue_index_w++;
                    packet_queue_put_nullpacket(&is->subtitleq, is->subtitle_stream,is->queue_index_w);
                }
                is->eof = 1;
            }
            if (pb_error) {
        ...
        if (pkt->flags & AV_PKT_FLAG_DISCONTINUITY) {
            if (is->audio_stream >= 0) {
                is->queue_index_w++;
                flush_pkt.index = is->queue_index_w;
                packet_queue_put(&is->audioq, &flush_pkt);
            }
            if (is->subtitle_stream >= 0) {
                is->queue_index_w++;
        ...
        enum AVMediaType media_type = ic->streams[pkt->stream_index]->codecpar->codec_type;
        is->queue_index_w++;
        if(count %100 == 0)
            av_log(NULL, AV_LOG_DEBUG,"packet_queue_put----> stream_start_time=%lld pkt_in_play_range=%d nb_streams=%d media_type=%d",stream_start_time,pkt_in_play_range,ic->nb_streams,media_type);

        /*下面增加pts 跳变处理，以及mpegts出现切换pid问题也就是pat变了pmt也变了*/
        if(ffp->avplayer_enable && is->protocal_type != AV_STREAM_PROTOCAL_TYPE_HLS && init_nb_streams < ic->nb_streams)
        {
            //av_log(NULL,AV_LOG_DEBUG,"ffp_set_stream_selected need_change_start_pts_flag=%d stream_type=%d",need_change_start_pts_flag,stream_type);
            if(need_change_start_pts_flag && (media_type ==AVMEDIA_TYPE_AUDIO || media_type ==AVMEDIA_TYPE_VIDEO))
            {
                //av_log(NULL,AV_LOG_DEBUG,"ffp_set_stream_selected stream_start_time=%lld is_seek_change_pts=%d hw_decode_pts=%lld",
               //            stream_start_time,is_seek_change_pts,ffp->is->hw_decode_pts);
                if(pkt_ts != AV_NOPTS_VALUE)
                {
                    
                    if(is_seek_change_pts){
                        //if(stream_protocal_type == AV_STREAM_PROTOCAL_TYPE_RTSP){
                            //start_time = pts*1000/90
                           // is->ic->start_time = (stream_start_time -pts_change_seek_pos/1000*90*90/1000);
                            is->ic->start_time = (av_rescale(pkt_ts,AV_TIME_BASE,90000) -pts_change_seek_pos - is->ic->start_time);
                        //}
                        //else{
                        //    is->ic->start_time = (av_rescale(pkt_ts,AV_TIME_BASE,90000) -(pts_change_seek_pos - is->ic->start_time));
                        //}
                        av_log(NULL,AV_LOG_DEBUG,"ffp_set_stream_selected pkt_ts=%lld pts_change_seek_pos=%lld start_time=%lld",
                                   pkt_ts,pts_change_seek_pos,is->ic->start_time);
                        need_change_start_pts_flag = 0;
                        is_seek_change_pts =0;
                    }
                    else if(media_type ==AVMEDIA_TYPE_AUDIO){
                            av_log(NULL,AV_LOG_DEBUG,"ffp_set_stream_selected skip_pts_count=%d pts_change_after_current_pts=%lld stream_start_time=%lld pkt_ts=%lld",
                                        skip_pts_count,pts_change_after_current_pts,stream_start_time,pkt_ts);
                            if(skip_pts_count == 0 && pts_change_after_current_pts == -1L){
                                    long current_time = ffp_get_current_position_l(ffp);
                                   pts_change_after_current_pts = current_time;
                            }
                            else if(++skip_pts_count && (skip_pts_count == 20||(stream_start_time != AV_NOPTS_VALUE && pkt_ts - stream_start_time < 1000) ) ){
                                if(skip_pts_count == 20)
                                    is->ic->start_time =av_rescale(((stream_start_time != AV_NOPTS_VALUE)?stream_start_time:pkt_ts),AV_TIME_BASE,90000) - pts_change_after_current_pts*1000;
                                else
                                    is->ic->start_time = av_rescale(stream_start_time,AV_TIME_BASE,90000) - pts_change_after_current_pts*1000;
                                av_log(NULL,AV_LOG_DEBUG,"ffp_set_stream_selected skip_pts_count=%d pts_change_after_current_pts=%lld start_time=%lld pts=%lld stream_start_time=%lld",
                                           skip_pts_count,pts_change_after_current_pts,is->ic->start_time,pkt_ts,stream_start_time);
                                skip_pts_count =0;
                                pts_change_after_current_pts =-1L;
                                need_change_start_pts_flag = 0;
                            }
                        }
                        //av_log(NULL,AV_LOG_DEBUG,"ffp_set_stream_selected skip_pts_count=%d pkt_ts=%lld hw_decode_pts=%lld",
                        //               skip_pts_count,pkt_ts,ffp->is->hw_decode_pts);
                  }
            }

            if((media_type ==AVMEDIA_TYPE_AUDIO &&  pkt->stream_index != is->audio_stream)
                ||(media_type ==AVMEDIA_TYPE_VIDEO &&  pkt->stream_index != is->video_stream))
            {
                long current_time = ffp_get_current_position_l(ffp);
                ffp_set_stream_selected(ffp, pkt->stream_index, 1);

                need_change_start_pts_flag = 1;
                if(ffp->is->hw_decoder_seek)
                    is_seek_change_pts =1;
                else{
                        if(pts_change_after_current_pts == -1L || pts_change_after_current_pts == 0L){
                           skip_pts_count = 0;
                           pts_change_after_current_pts = current_time;
                        }
                        av_log(NULL,AV_LOG_DEBUG,"ffp_set_stream_selected current_time=%ld pts_change_after_current_pts=%lld",current_time,pts_change_after_current_pts);
                }
                //av_log(NULL,AV_LOG_DEBUG,"ffp_set_stream_selected pkt_ts=%lld hw_decoder_seek=%d need_change_start_pts_flag=%d is_seek_change_pts=%d",
                //                   pkt_ts,ffp->is->hw_decoder_seek,need_change_start_pts_flag,is_seek_change_pts);
            }

        }

        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
            pkt->index = is->queue_index_w;
...
fail:
    ...
    av_log(NULL, AV_LOG_ERROR,"read_thread -------------->exit");
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    av_log(NULL,AV_LOG_DEBUG,"SDL_CreateThreadEx ff_read");
...
void ffp_set_option_int(FFPlayer *ffp, int opt_category, const char *name, int64_t value)
{
    if (!ffp)
        return;
    if(FFP_OPT_CATEGORY_AVPLAYER == opt_category) {
        ffp->avplayer_enable = 1;
        return ;
    }
    AVDictionary **dict = ffp_get_opt_dict(ffp, opt_category);
...
int ffp_get_video_codec_info(FFPlayer *ffp, char **codec_info)
{
    ...
    if (ffp->video_codec_info) {
        *codec_info = strdup(ffp->video_codec_info);
        av_log(ffp, AV_LOG_DEBUG, "getVideoCodec: %s\n", ffp->audio_codec_info);
    } else {
        *codec_info = NULL;
    }
    return 0;
...
int ffp_start_l(FFPlayer *ffp)
{
    ...
    if (!is)
        return EIJK_NULL_IS_PTR;
    if(ffp->avplayer_enable) {
        int mode = HJ_GetPlayMode();
        av_log(NULL,AV_LOG_DEBUG,"HJ_GetPlayMode mode=%d",mode);
        if(mode == 4  /*HI_PLAYER_STATE_PAUSE*/)
            HJ_Resume();
    }
    toggle_pause(ffp, 0);
    return 0;
...
int ffp_seek_to_l(FFPlayer *ffp, long msec)
{
    ...
    if((is->stream_type == AV_STREAM_TIMESHIFT || is->stream_type == AV_STREAM_LIVE) && is->protocal_type == AV_STREAM_PROTOCAL_TYPE_RTSP)
        msec = ffp_get_duration_l(ffp) -msec;
    else if((is->stream_type == AV_STREAM_TIMESHIFT || is->stream_type == AV_STREAM_LIVE) && is->protocal_type == AV_STREAM_PROTOCAL_TYPE_HLS)
        msec = ffp_get_duration_l(ffp) -msec;
    ...
    //if( ffp && strstr(ffp->input_filename,"rtsp://")) {
    //    ;
    //} else {
...
int update_hw_decode_pts(VideoState *is,int64_t* _pts)
{
    int64_t  pts = *_pts;
    int64_t tmp_pts = -1LL;
    *_pts = -1LL;
    if(!is && pts != 0xffffffffU && !_pts)
        return -1;
    tmp_pts = HW_DECODER_PTS_MAX;
    //av_log(NULL,AV_LOG_DEBUG,"update_hw_not_decode 1 tmp_pts=%lld",tmp_pts);
    tmp_pts *=  is->big_number_pts;
    //av_log(NULL,AV_LOG_DEBUG,"update_hw_not_decode 2 tmp_pts=%lld",tmp_pts);
    tmp_pts = (is->big_number_pts != -1LL) ? pts*90+ tmp_pts : is->big_number_pts ;
    //av_log(NULL,AV_LOG_DEBUG,"update_hw_not_decode 3 tmp_pts=%lld",tmp_pts);
    is->hw_decode_pts =tmp_pts/90;
    if(count %100 == 0)
    av_log(NULL,AV_LOG_DEBUG,"update_hw_decode 4 is->hw_decode_pts=%lld",is->hw_decode_pts);
    *_pts = is->hw_decode_pts  ;
    //av_log(NULL,AV_LOG_DEBUG,"update_hw_not_decode 5 *_pts=%lld",*_pts);
    return 0;
}
long ffp_get_current_position_l(FFPlayer *ffp)
{
    ...
    if(ffp->avplayer_enable) {
        count++;
        int64_t current_pts = HJ_GetCurrentPts();
         if(count %100 == 0)
        av_log(NULL,AV_LOG_DEBUG,"%s %d current_pts=%lld start_time=%lld start_diff=%lld ret_time=%ld ",__FUNCTION__,__LINE__,current_pts,start_time,start_diff,ret_time);
        if(current_pts == -1)
            return -1;
        update_hw_decode_pts(is, &current_pts);
        if(current_pts == -1)
            return -1;
        ret_time = (ret_time = current_pts - start_diff)>0?ret_time:-1L;
        // av_log(NULL,AV_LOG_DEBUG,"%s %d current_pts=%lld start_diff=%lld ret_time=%ld",__FUNCTION__,__LINE__,current_pts,start_diff,ret_time);
        
        if(count %100 == 0)
                     av_log(NULL,AV_LOG_DEBUG,"%s %d current_pts=%lld start_diff=%lld ret_time=%ld big_number_pts=%d hw_decode_pts=%lld hw_not_decode_pts=%lld",__FUNCTION__,__LINE__,current_pts,start_diff,ret_time,is->big_number_pts,is->hw_decode_pts,is->hw_not_decode_pts);
        if(is->protocal_type == AV_STREAM_PROTOCAL_TYPE_RTSP)
        {
             if(is->stream_type == AV_STREAM_LIVE)
                return fftime_to_milliseconds(is->ic->duration);
             else if(is->stream_type == AV_STREAM_TIMESHIFT)
                return fftime_to_milliseconds(is->ic->duration) - is->timeshift_range_live_time;
        }
        else if(is->protocal_type == AV_STREAM_PROTOCAL_TYPE_HLS)
        {
             if(is->stream_type == AV_STREAM_LIVE)
...
long ffp_get_duration_l(FFPlayer *ffp)
{
    ...
    //av_log(NULL,AV_LOG_DEBUG,"ffp_get_duration_l duration=%lld",duration);
    if (duration < 0)
        return 0;
...
void ffp_check_buffering_l(FFPlayer *ffp)
{
    ...
    if(ffp->avplayer_enable) {
        if(is->hw_decode_pts != -1LL && is->hw_not_decode_pts !=-1LL) {
            is->hw_decode_cache_time = is->hw_not_decode_pts - is->hw_decode_pts;
        } else
            is->hw_decode_cache_time = -1LL;
        //av_log(ffp, AV_LOG_DEBUG, "hw_decode_pts=%lld hw_not_decode_pts=%lld hw_decode_cache_time=%lld",
        //                           is->hw_decode_pts,is->hw_not_decode_pts, is->hw_decode_cache_time);
    }
    if (hwm_in_ms > 0) {
        ...
        if (is->audio_st && audio_time_base_valid) {
            if(ffp->avplayer_enable)
                audio_cached_duration = ffp->stat.audio_cache.duration+((is->hw_decode_cache_time != -1LL)?is->hw_decode_cache_time:0);
            else
        ...
        if (is->video_st && video_time_base_valid) {
            if(ffp->avplayer_enable)
                video_cached_duration = ffp->stat.video_cache.duration+((is->hw_decode_cache_time != -1LL)?is->hw_decode_cache_time:0);
            else
    ...
    //int64_t tcp_speed = SDL_SpeedSampler2GetSpeed(&ffp->stat.tcp_read_sampler)*8 ;
    //av_log(ffp, AV_LOG_DEBUG,"tcp speed =%lld",tcp_speed);
    if (need_start_buffering) {
        ...
        if(ffp->avplayer_enable) {
            if (is->buffer_indicator_queue && is->buffer_indicator_queue->nb_packets > 0) {
                if(is->paused && !is->hw_decoder_seek) {
                    if (   (is->audioq.nb_packets >= BUFF_END_MIN_FRAMES || is->audio_stream < 0 || is->audioq.abort_request)
                           && (is->videoq.nb_packets >= BUFF_END_MIN_FRAMES || is->video_stream < 0 || is->videoq.abort_request)) {
                        ffp_toggle_buffering(ffp, 0);
                        av_log(ffp, AV_LOG_DEBUG, "paused hw_decode_pts=%lld hw_not_decode_pts=%lld hw_decode_cache_time=%lld",
                               is->hw_decode_pts,is->hw_not_decode_pts, is->hw_decode_cache_time);
                        av_log(ffp, AV_LOG_DEBUG, "ffp_toggle_buffering 0 %d %d %d %d",is->buffer_indicator_queue,is->buffer_indicator_queue->nb_packets,is->audioq.nb_packets,is->videoq.nb_packets);
                    }
                    ...
                    if (   (is->audioq.nb_packets >= MIN_MIN_FRAMES || is->audio_stream < 0 || is->audioq.abort_request)
                        ...
                        av_log(ffp, AV_LOG_DEBUG, "hw_decoder_seek ffp_toggle_buffering 0 %d %d %d %d",is->buffer_indicator_queue,is->buffer_indicator_queue->nb_packets,is->audioq.nb_packets,is->videoq.nb_packets);
                    }
                }
            }
        } else {
            if (is->buffer_indicator_queue && is->buffer_indicator_queue->nb_packets > 0) {
                if (   (is->audioq.nb_packets >= SEEK_AFTER_MIN_FRAMES || is->audio_stream < 0 || is->audioq.abort_request)
                       && (is->videoq.nb_packets >= SEEK_AFTER_MIN_FRAMES || is->video_stream < 0 || is->videoq.abort_request)) {
                    ffp_toggle_buffering(ffp, 0);
                    //av_log(ffp, AV_LOG_DEBUG, "ffp_toggle_buffering 0 %d %d %d %d",is->buffer_indicator_queue,is->buffer_indicator_queue->nb_packets,is->audioq.nb_packets,is->videoq.nb_packets);
                }
            }
...
void ffp_set_video_codec_info(FFPlayer *ffp, const char *module, const char *codec)
{
    ...
    av_log(ffp, AV_LOG_DEBUG, "VideoCodec: %s\n", ffp->video_codec_info);
}
...
void ffp_set_audio_codec_info(FFPlayer *ffp, const char *module, const char *codec)
{
    ...
    av_log(ffp, AV_LOG_DEBUG, "AudioCodec: %s\n", ffp->audio_codec_info);
}
...
int ffp_set_stream_selected(FFPlayer *ffp, int stream, int selected)
{
    ...
    av_log(ffp, AV_LOG_DEBUG,"ffp_set_stream_selected codec_type=%d codec_name=%s",codecpar->codec_type,avcodec_get_name(codecpar->codec_id));
    if (selected) {
        switch (codecpar->codec_type) {
        case AVMEDIA_TYPE_VIDEO:
