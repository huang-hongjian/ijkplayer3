...
#include "../hjplayer_adp/hjplayer_adp.h"
...
#define SEEK_AFTER_MIN_FRAMES      2
#define BUFF_END_MIN_FRAMES      40
#define MAX_MIN_FRAMES      50000
#define HW_DECODER_CACHE_TIME      (350LL) //200ms   ´¥·¢»º³åÊÂ¼þµÄ×îÐ¡½âÂëÆ÷»º´æÊ±¼ä
#define BUFFERING_START_DELAY      (50000) //50ms   ¹ýÂË5msÒÔÄÚµÄ»º³åÊÂ¼þ
#define BUFFERING_END_AFTER_REPORT_DELAY 2000000 //2s
#define COMPLETED_DELAY (100L) //100 ms
#define COMPLETED_RANGE (1500L) //1.5 s
#define COMPLETED_DELAY_COUNT (100)
#define BUFFERING_STATUS_RANGE 2000000 //2s
#define CHECK_EMPTY_BUFFER_MIN_NUM 5
#define RTSP_TIMESHIFT_DURATION (3600000)
#define HLS_TIMESHIFT_DURATION (2*3600000)
...
typedef enum {
     AV_STREAM_PROTOCAL_TYPE_NONE, 
    AV_STREAM_PROTOCAL_TYPE_HLS,
    AV_STREAM_PROTOCAL_TYPE_RTSP,
    AV_STREAM_PROTOCAL_TYPE_HTTP,
    AV_STREAM_PROTOCAL_TYPE_UDP,
    AV_STREAM_PROTOCAL_TYPE_IGMP,
}AVProtocalType;
...
typedef struct HalPacket_ {
    PLAYER_STREAMTYPE_E  type;		/* ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Í£ï¿½vid/aud/sub */
    uint32_t      stream_index;		/* ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½index */
    int         is_key_frame;
    uint32_t      len;
    uint8_t*      data;
    uint32_t      header_len;
...
typedef struct VideoState {
    ...
    int queue_index_w;
    int queue_index_r;
    int64_t hw_decode_pts;
    int64_t hw_not_decode_pts;
    int64_t hw_decode_cache_time;
    int big_number_pts;
    int hw_decoder_seek;
    int64_t hw_buffering_start_time;
    int64_t hw_buffering_end_time;
    int is_completed;
    int64_t hw_seek_pos;
    AVStreamType stream_type;
    AVProtocalType protocal_type;
    int64_t timeshift_range_live_time;
//    int64_t sync_pts;
} VideoState;
...
#define HICODEC_MODULE_NAME "HiCodec"
#define AMCODEC_MODULE_NAME "AmCodec"
#endif
