...
long ijkmp_get_current_position(IjkMediaPlayer *mp)
{
    ...
    av_log(NULL,AV_LOG_DEBUG,"%s %ld seek_req=%d seek_msec=%ld retval=%ld",__FUNCTION__,__LINE__,mp->seek_req,mp->seek_msec,retval);
	if(retval==-1L)
	{	
       av_log(NULL,AV_LOG_DEBUG,"%s %ld  seek_msec=%d",__FUNCTION__,__LINE__,mp->seek_msec);


         if(  retval == -1L && mp->ffplayer && mp->ffplayer->is && mp->ffplayer->is->hw_decoder_seek != -1)
         {
               retval = fftime_to_milliseconds(mp->ffplayer->is->hw_seek_pos);
               
               av_log(NULL,AV_LOG_DEBUG,"%s %ld  seek_msec=%d hw_seek_pos=%lld retval=%ld",__FUNCTION__,__LINE__,mp->seek_msec,mp->ffplayer->is->hw_seek_pos,retval);
...
int ijkmp_get_msg(IjkMediaPlayer *mp, AVMessage *msg, int block)
{
    ...
    while (1) {
        ...
        case FFP_MSG_SEEK_COMPLETE:
            ...
            break;
        case FFP_MSG_SEEK_AFTER_FIRST_FRAME:
            MPTRACE("ijkmp_get_msg: FFP_MSG_SEEK_AFTER_FIRST_FRAME\n");
            pthread_mutex_lock(&mp->mutex);
	     mp->seek_msec_hal = -1L;
            pthread_mutex_unlock(&mp->mutex);
			break;
