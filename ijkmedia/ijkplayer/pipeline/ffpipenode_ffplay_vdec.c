...
IJKFF_Pipenode *ffpipenode_create_video_decoder_from_ffplay(FFPlayer *ffp)
{
    ...
    if(ffp->avplayer_enable)
    {
        if(ffp->is && ffp->is->hardware == AMLOGIC)
            ffp_set_video_codec_info(ffp, AMCODEC_MODULE_NAME, avcodec_get_name(ffp->is->viddec.avctx->codec_id));
        else if(ffp->is &&ffp->is->hardware == HISI)
            ffp_set_video_codec_info(ffp, HICODEC_MODULE_NAME, avcodec_get_name(ffp->is->viddec.avctx->codec_id));
        else
